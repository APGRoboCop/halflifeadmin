...
return (!strncmp(string,pattern,strlen(string)));
...
if ( (int)CVAR_GET_FLOAT("admin_debug")!=0) {
	...
	if (encrypt==NULL || nicks[i].pass==NULL) return 0;
...
int check_model_pass(const char *model,const char *pass) {
	...
	if (model==NULL || pass==NULL) return 0;
#ifndef USE_MYSQL
	char *encrypt;
	...
	encrypt=(char *)&pass[0];
#endif
	if (encrypt==NULL || models[i].pass==NULL) return 0;
