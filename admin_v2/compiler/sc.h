 ...
 *  Copyright T. Riemersma, 1997-2001
 */
#ifndef __SC_H
#define __SC_H
#include <setjmp.h>
...
#if __MSDOS__ || defined __WIN32__ || defined __WINDOWS__ || defined __NT__ || defined _Windows
  #define DIRSEP_CHAR '\\'
#else
...
typedef struct __symbol {
  ...
  union {
    ...
    constval *lib;      /* function: library it is part of */
  } x;                  /* 'x' for 'extra' */
  union {
 ...
 *        1     (uREFER) the function is referred to ("used") in the source file
 *        2     (uRETVALUE) the function returns a value (or should return a value)
 *        3     (uPROTOTYPED) function was prototyped
 *        4     (uPUBLIC) function is public
 *        5     (uNATIVE) function is native
 *        6     (uSTOCK) function is discardable (without warning)
 *
 *  CONSTANT
 *  bits: 0     (uDEFINE) set 1 if the symbol is defined in the source file
 *        3     (uPREDEF) set if the constant is pre-defined and should be kept
 */
#define uDEFINE   0x01
#define uREAD     0x02
#define uWRITTEN  0x04
#define uCONST    0x08
#define uREFER    0x02  /* note: uREFER == uREAD */
...
int sc_error(int number,char *message,char *filename,int firstline,int lastline,void *argptr);
...
long sc_lengthbin(void *handle); /* length of the file */
...
SC_VDECL char includepath[];  /* directory for system include files */

SC_VDECL char ctrlchar;       /* the control character (or escape character) */
...
SC_VDECL cell glb_declared;   /* number of global bytes declared */
...
SC_VDECL cell sc_tabsize;     /* number of spaces that a TAB represents */
