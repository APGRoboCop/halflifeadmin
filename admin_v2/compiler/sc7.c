...
static void stgstring(char *start,char *end)
{
  ...
  while (start<end) {
    if (*start==sSTARTREORDER) {
      ...
      ptr=start;
      while (*ptr!=sSTARTREORDER && ptr<end)
        ptr+=strlen(ptr)+1;
      stgopt(start,ptr);
      start=ptr;
    } /* if */
...
SC_FUNC int phopt_init(void)
{
  ...
  if ((sequences=malloc(number * sizeof(SEQUENCE)))==NULL)
    return FALSE;
...
static void stgopt(char *start,char *end)
{
  ...
  while (start<end) {
    for (seq=0; sequences[seq].find!=NULL; seq++) {
      if ((debug & sNOOPTIMIZE)!=0)
        continue;       /* do not match anything if debugger is shut off */
