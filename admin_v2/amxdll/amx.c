...
static unsigned short *alignshort(unsigned short *v)
{
...
int AMXAPI amx_Init(AMX *amx,void *program)
{
  ...
  init_little_endian();
  if (!amx_LittleEndian) {
    aligncell((ucell *)&hdr->size);
    alignshort(&hdr->magic);
    alignshort((unsigned short *)&hdr->flags);
    alignshort((unsigned short *)&hdr->defsize);
    aligncell((ucell *)&hdr->cod);
    aligncell((ucell *)&hdr->dat);
    aligncell((ucell *)&hdr->hea);
    aligncell((ucell *)&hdr->stp);
    aligncell((ucell *)&hdr->cip);
    alignshort((unsigned short *)&hdr->num_publics);
    aligncell((ucell *)&hdr->publics);
    alignshort((unsigned short *)&hdr->num_natives);
    aligncell((ucell *)&hdr->natives);
    alignshort((unsigned short *)&hdr->num_libraries);
    aligncell((ucell *)&hdr->libraries);
    alignshort((unsigned short *)&hdr->num_pubvars);
    aligncell((ucell *)&hdr->pubvars);
    alignshort((unsigned short *)&hdr->num_tags);
    aligncell((ucell *)&hdr->tags);
  } /* if */

  if ((unsigned short)hdr->magic!=AMX_MAGIC || hdr->defsize!=sizeof(AMX_FUNCSTUB))
    return AMX_ERR_FORMAT;
  ...
  * (cell *)(amx->base+(int)hdr->stp) = 0;
  ...
  if (!amx_LittleEndian) {
    ...
    for (i=0; i<hdr->num_publics; i++) {
      aligncell((ucell *)&fs->address);
      fs+=sizeof(AMX_FUNCSTUB);
    } /* for */
...
int AMXAPI amx_NameLength(AMX *amx, int *length)
{
  AMX_HEADER *hdr=(AMX_HEADER *)amx->base;
  assert(hdr!=NULL);
  *length=hdr->defsize - sizeof(u_long);
  return AMX_ERR_NONE;
}
...
int AMXAPI amx_GetPublic(AMX *amx, int index, char *funcname)
{
  ...
  strncpy(funcname,func->name, sEXPMAX+1);
  return AMX_ERR_NONE;
}
  ...
  op_strb_i:
    ...
    case 1:
      *(data+(int)alt)=(u_char)pri;
      break;
    case 2:
      *(u_short *)(data+(int)alt)=(u_short)pri;
      break;
    case 4:
      *(cell *)(data+(int)alt)=pri;
      break;
 ...
 * amx_Exec(), but can pass up to 4 parameters.
 */
int AMXAPI amx_Execv(AMX *amx, cell *retval, int index, int numparams, cell params[])
{
  switch (numparams) {
  case 0:
    return amx_Exec(amx, retval, index, numparams);
  case 1:
    return amx_Exec(amx, retval, index, numparams, params[0]);
  case 2:
    return amx_Exec(amx, retval, index, numparams, params[0], params[1]);
  case 3:
    return amx_Exec(amx, retval, index, numparams, params[0], params[1], params[2]);
  case 4:
    return amx_Exec(amx, retval, index, numparams, params[0], params[1], params[2], params[3]);
  default:
