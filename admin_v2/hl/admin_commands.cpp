/*
 * $Id: admin_commands.cpp,v 1.44 2001/09/27 20:33:16 darope Exp $
 *
 *
...
static cell centersay(AMX *amx, cell *params) {
  ...
  FormatLine(sText);
  iLineFeed = wrap_lines( sText, CENTER_SAY_LINE_SIZE, SW_NOWRAP);
  if ( !iLineFeed ){
...
static cell selfmessage(AMX *amx, cell *params) {
  ...
  CHECK_AMX_PARAMS(1);
  GET_AMX_STRING(1,BUF_SIZE,sText,1);
 ...
 * Revision 1.44  2001/09/27 20:33:16  darope
 * This is the merged version between 2.50e and the 2.51 branch from
 * admin_muck. It does compile, but is untested if it also works.
 * It has the AMX from 01-04-2001 and the Small compiler 1.6.
 *
 ...
 *
 * Revision 1.23.2.4  2001/05/24 21:53:17  darope
 * Fade-in time for centersay is half the display time
 *
 * Revision 1.23.2.3  2001/05/22 09:19:24  darope
 * -- Added line length checks to functions messageex(), tsay() and rainbow().
 * -- Added centersayex() function to print centersay messages to only one
 *    user.
 *
 * Revision 1.23.2.2  2001/05/21 15:12:48  darope
 * -- Version information moved to version.h and version.cpp
 * -- Stack scribble fixed in GET_AMX_STRING
 * -- GET_AMX_STRING truncates, it has a new parameter to control it
 * -- replaced a few debug messages with more performant DEBUG_LOG
 * -- new macro DEBUG_DO to execute code conditionally
 * -- IsPlayerValid made inline
 * -- "Compiled:" message added to mod info
 * -- UTIL_LogPrintf got vsnprintf instead of vsprintf
 *
 * Revision 1.23.2.1  2001/05/14 22:51:08  darope
 * A few bug fixes and a new AMX version
 *
