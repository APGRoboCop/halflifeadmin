/*
 * $Id: amutil.cpp,v 1.1 2001/09/27 20:33:16 darope Exp $
 *
 *
 ...
 *              source - header relationship, but includinh util.h keeps giving
 *              me trouble. Thus I start to move utility functions over here and
 *              declaring them in amutil.h. I sstart with function completely
 *              unrelated to adminmod. For now, functions that use AM or HL 
 *              specififc data structures of functions are kept in, well, wherever
 *              they are now. =)
 *
 */
...
int wrap_lines( char* _pcString, int _iLineLen, int _iWrap ) {
  ...
  while ( pcLineStart < pcStringEnd ) {
	...
	while ( *pcLineEnd != ' ' && pcLineEnd > _pcString ) pcLineEnd--;
	if ( *pcLineEnd == ' ' ) {
	  *pcLineEnd = '\n';
