/*
 * $Id: admin_mod.cpp,v 1.2 2001/09/27 20:33:16 darope Exp $
 *
 *
...
cvar_t admin_reconnect_timeout = {"admin_reconnect_timeout","300",FCVAR_EXTDLL, 300};
...
cvar_t admin_vault_file = {"admin_vault_file","vault.ini",FCVAR_EXTDLL};
...
cvar_t pretty_say = {"pretty_say","1",FCVAR_EXTDLL, 1};
...
int AM_ClientCommand( edict_t *pEntity ) {
  ...
  DEVEL_LOG(2, ("AM_ClientCommand finished.\n") );
...
BOOL AM_ClientConnect( edict_t *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[ 128 ] ) {    
  ...
  DEBUG_LOG(1, ("AM_ClientConnect: %s\n", sName) );
  ...
  */
  ...
  if ((int)CVAR_GET_FLOAT("admin_debug")!=0) {
    UTIL_LogPrintf("[ADMIN] %i free spots / %i max spots.\n",iFreeSlots,gpGlobals->maxClients);
  }
...
void AM_ClientStart(edict_t *pEntity) {
  ...
  /* fetch the admin_debug cvar so we can use it directly */
  ptAM_debug = 0;
...
void* LoadScript(AMX *amx,char *filename) {
  ...
  #define CUR_FILE_VERSION 5
    ...
    if ( hdr.file_version < CUR_FILE_VERSION ) {
      UTIL_LogPrintf( "[ADMIN] ERROR: The script binary has an outdated format. (version %i)\n", hdr.file_version );
      UTIL_LogPrintf( "[ADMIN]        Please recompile the script with the compiler\n" );
