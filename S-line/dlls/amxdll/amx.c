 ...
 *
 *  Version: $Id: amx.c,v 1.4 2004/07/23 21:55:10 darope Exp $
 */
...
#if defined __LP64__
#  define amx_AlignFP amx_Align64
#else
#  define amx_AlignFP amx_Align32
#endif
...
//printf( "DBG: hdr->magic = 0x%x, AMX_MAGIC = 0x%x | hdr->defsize = %d, sizeof(AMX_FUNCSTUB) = %d\n", hdr->magic, AMX_MAGIC, hdr->defsize, sizeof(AMX_FUNCSTUB) );
    return AMX_ERR_FORMAT;
  }
...
static void *labels[] = {
  ...
  if (index!=AMX_EXEC_CONT) {
    ...
    if (numparams & 0xFFFF0000) {
      ...
      va_start(ap,numparams);
      for (i=0; i<numparams; i++) {
	  	cell arg = va_arg(ap,cell);
        *(cell *)(data+(int)stk+i*sizeof(cell))=arg;
	  }
      va_end(ap);
    } /* if */
  ...
  op_movs:
    GETPARAM(offs);
    memcpy(data+(int)alt, data+(int)pri, (size_t)offs);
    NEXT(cip);
  op_cmps:
    GETPARAM(offs);
    pri=memcmp(data+(int)alt, data+(int)pri, (size_t)offs);
    NEXT(cip);
  op_fill:
    GETPARAM(offs);
