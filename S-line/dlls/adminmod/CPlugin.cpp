...
BOOL CPlugin::CheckCommand(const char* _pcCommand, unsigned int& _uiAccess)
{
...
plugin_result CPlugin::HandleCommand(edict_t* pEntity, char* sCmd, char* sData)
{
    ...
    // Some safekeeping. A NULL sCmd gets passed as a an empty string. Same for sData.
    // DOS line endings get transformed to proper format without the CR.
    if(sCmd == NULL) {
        sCmd = "";
    } else {
        char* pcCR = NULL;
        // check if the first '\r' from right is the last ("\r\0") or second to last (prob. "\r\l\0") character. If yes,
        // end the string there.
        if((pcCR = strrchr(sCmd, '\r')) != NULL && (*(pcCR + 1) == '\0' || *(pcCR + 2) == '\0'))
            *pcCR = '\0';
    } // if
    ...
    if(sData == NULL) {
        ...
        // check if the first '\r' from right is the last ("\r\0") or second to last (prob. "\r\l\0") character. If yes,
        // end the string there.
        if((pcCR = strrchr(sData, '\r')) != NULL && (*(pcCR + 1) == '\0' || *(pcCR + 2) == '\0'))
            *pcCR = '\0';
    } // if
    ...
    if(m_iEventCommandIndex != INVALID_INDEX) {
        ...
        if(pEntity == NULL) {
            iError =
                amx_Exec(m_pAmx, &iResult, m_iEventCommandIndex, 4, (cell)sCmd, (cell)sData, (cell) "Admin", (cell)0);
        } else {
        ...
        if(iError != AMX_ERR_NONE) {
            UTIL_LogPrintf(
                "[ADMIN] ERROR: Plugin %s returned error %i when executing plugin_command\n", m_sFile, iError);
            if(pEntity != NULL) {
                CLIENT_PRINTF(pEntity, print_console,
                    UTIL_VarArgs("[ADMIN] ERROR: Plugin %s returned error %i when executing plugin_command\n", m_sFile,
                                  iError));
            }
            return PLUGIN_ERROR;
        }
    ...
    while(pLink != NULL) {
        ...
        if(stricmp(sCmd, pCmd->sCmd) == 0) {
            DEBUG_LOG(3, ("Match found on command %s (access:%i) for plugin %s / function %i", sCmd, pCmd->iAccess,
                             m_sFile, pCmd->iIndex));
            break;
        }
        pLink = pLink->NextLink();
    ...
    if(pEntity == NULL) {
        ...
        iError = amx_Exec(m_pAmx, &iResult, pCmd->iIndex, 4, (cell)sCmd, (cell)sData, (cell)STRING(pEntity->v.netname),
            (cell)ENTINDEX(pEntity));
    }
...
plugin_result CPlugin::HandleDisconnect(edict_t* pEntity)
{
    ...
    if(m_pAmx == NULL) {
        UTIL_LogPrintf(
            "[ADMIN] ERROR: CPlugin::HandleDisconnect called when no AMX present for plugin '%s'.\n", m_sFile);
        return PLUGIN_ERROR;
        // Verify that we have a valid entity
    } else if(pEntity == NULL) {
    ...
    if(iError != AMX_ERR_NONE) {
        UTIL_LogPrintf(
            "[ADMIN] ERROR: Plugin %s returned error %i when executing plugin_disconnect\n", m_sFile, iError);
        return PLUGIN_ERROR;
    }
...
plugin_result CPlugin::HandleInfo(edict_t* pEntity, char* sNewName)
{
    ...
    iError = amx_Exec(
        m_pAmx, &iResult, m_iEventInfoIndex, 3, (cell)STRING(pEntity->v.netname), (cell)sNewName, (cell)iIndex);
    // Check for errors.
    if(iError != AMX_ERR_NONE) {
...
int CPlugin::LoadFile(char* sFilename)
{
    ...
    if((fp = fopen(sFilename, "rb")) != NULL) {
        ...
        if(iFileType == INVAL_AMX_HDR) {
            ...
            UTIL_LogPrintf(
                "[ADMIN] ERROR: CPlugin::LoadFile: File '%s' is not a valid Admin Mod script binary.\n", sFilename);
            fclose(fp);
            return 0;
            ...
            UTIL_LogPrintf(
                "[ADMIN] ERROR: CPlugin::LoadFile: File '%s' has a file version not supported by Admin Mod (%i,%i).\n",
...
#else
            // This is a AMX file format that we do not support
            UTIL_LogPrintf("[ADMIN] ERROR: CPlugin::LoadFile: File '%s' has a format not supported by this version of "
                           "Admin Mod: 64-bit script binary.\n",
                sFilename);
            fclose(fp);
            return 0;
#endif
            ...
            // Should not happen.
            UTIL_LogPrintf(
                "[ADMIN] ERROR: CPlugin::LoadFile: Unexpected error on checking version of file '%s', cannot load.\n",
                sFilename);
            fclose(fp);
            return 0;
        } // if-else
        ...
        if(m_pProgram != NULL) {
            ...
            if(!bFileNeedsConv) {
                ...
                long lCodeSize;
                if(pTmpMemFile == NULL) {
                    UTIL_LogPrintf("[ADMIN] ERROR: CPlugin::LoadFile: Loading file '%s' to memory failed: not enough "
                                   "memory (%i)\n",
                        sFilename, hdr.size);
                    delete[] m_pProgram;
                    fclose(fp);
...
// convert the header, storing it into the program memory
#ifdef LINUX
                DEBUG_LOG(
                    2, ("CPlugin::LoadFile: Loading Win32 script file '%s', converting to Linux format.", sFilename));
...
#else
                DEBUG_LOG(
                    2, ("CPlugin::LoadFile: Loading Linux script file '%s', converting to Win32 format.", sFilename));
                if(hdr.file_version == 4) {
            ...
            if(iError != AMX_ERR_NONE && iError != AMX_ERR_NOTFOUND) {
                UTIL_LogPrintf(
                    "[ADMIN] ERROR: CPlugin::LoadFile: Call to amx_Init on plugin '%s' returned error #%i.\n",
                    sFilename, iError);
                delete[] m_pProgram;
                return 0;
            }
