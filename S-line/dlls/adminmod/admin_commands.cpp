...
#define CHECK_AMX_PARAMS(Required)                                                                                     \
    if(iNumParams < Required) {                                                                                        \
        System_Error(                                                                                                  \
            UTIL_VarArgs("[ADMIN] ERROR: Missing required parameters.  Required %i, has %i.\n", Required, iNumParams), \
            pAdminEnt);                                                                                                \
...
#define GET_AMX_STRING(Param, MaxLength, String, Truncate)                                                        \
    ...
    if(iLength >= MaxLength) {                                                                                    \
        if(Truncate == 0) {                                                                                       \
            System_Error(                                                                                         \
                UTIL_VarArgs("[ADMIN] ERROR: AMX string too large.  Max size %i, is %i.\n", MaxLength, iLength),  \
                pAdminEnt);                                                                                       \
            amx_RaiseError(amx, AMX_ERR_NATIVE);                                                                  \
            return 0;                                                                                             \
        } else {                                                                                                  \
...
static cell access(AMX* amx, cell* params)
{
    ...
    if(strlen(sUser) > 0) {
        ...
        if(iAccess < 0)
            return (ucell)GetUserAccess(pAdminEnt);
...
static cell ban(AMX* amx, cell* params)
{
    ...
    if((params[3] == c_iBanByIP || params[3] == c_iBanBoth) && bIsIP == false) {
        if(user_ip(iPlayerIndex, &pcUser)) {
            ...
            System_Response(
                UTIL_VarArgs("[ADMIN] (ban) IP lookup for player '%s' failed. Cannot ban by IP.\n", sUser), pAdminEnt);
            if(params[3] == c_iBanByIP)
                return 0;
        } // if-else
    }     // if
    ...
    if(params[3] == c_iBanBoth && !oaiAuthID.is_set()) {
        System_Response(
            "[ADMIN] (ban) Cannot ban both WONID and IP when the IP was provided. Use a name or ID.\n", pAdminEnt);
        return 0;
    } // if
...
static cell centersayex(AMX* amx, cell* params)
{
    ...
    if(!IsPlayerValid(pPlayer)) {
        ...
        if(!iLineFeed) {
            System_Response(
                UTIL_VarArgs("[ADMIN] (centersayex) The maximum size for any one line in a centersay is %i.\n",
                    CENTER_SAY_LINE_SIZE),
                pAdminEnt);
        }
        if(pAdminEnt != NULL) {
...
static cell changelevel(AMX* amx, cell* params)
{
    ...
    if(iIPause > 0) {
        ...
        if(iTimer == INVALID_TIMER) {
            UTIL_LogPrintf(
                "[ADMIN] ERROR (changelevel): Could not set timer for intermission. Changing map directly.\n");
        } else {
...
bool is_registering_command(const char* _pcCommand)
{
    ...
    if(_pcCommand == NULL || strchr(_pcCommand, ' ') == 0)
        return false;
    ...
    if(pcRegCmds != NULL) {
        ...
        while((pcEnd - pcStart) > 0) {
            if(*pcEnd == ' ') {
                ...
                if((strncasecmp(_pcCommand, pcStart, (pcEnd - pcStart)) == 0) && ((pcEnd - pcStart) <= iCmdLen) &&
                    (_pcCommand[pcEnd - pcStart] == ' ')) {
                    return true;
                } // if
                pcStart = pcEnd;
            } // if-else
...
static cell exec(AMX* amx, cell* params)
{
    ...
    if((pcPos = strstr(sCommand, "admin_command")) != 0) {
        ...
        if(iSemicolons != 0) {
            ...
            System_Response(
                "[ADMIN] (exec) You cannot use exec() containing 'admin_command' and semicolons.\n", pAdminEnt);
            return 0;
...
static cell kill_timer(AMX* amx, cell* params)
{
    ...
    if(pEntity->GetTimerAMX(iTimer) != amx) {
        UTIL_LogPrintf(
            "[ADMIN] ERROR: A plugin is attempting to kill a timer index %i that it does not own.\n", iTimer + 1);
        amx_RaiseError(amx, AMX_ERR_NATIVE);
        return 0;
    } // if
...
static cell messageex(AMX* amx, cell* params)
{
    ...
    if(!IsPlayerValid(pPlayer)) {
        ...
        if(!wrap_lines(sText, CENTER_SAY_LINE_SIZE, SW_WRAP)) {
            System_Response(
                UTIL_VarArgs("[ADMIN] (messageex) The maximum size for any one line in a print_tty is %i.\n",
                    CENTER_SAY_LINE_SIZE),
                pAdminEnt);
            if(pAdminEnt != NULL) {
        ...
        for(i = 0; i < (int)strlen(sText); i++) {
            if((i - iLineFeed) >= CENTER_SAY_LINE_SIZE) {
                System_Response(
                    UTIL_VarArgs("[ADMIN] (messageex) The maximum size for any one line in a centersay is %i.\n",
                        CENTER_SAY_LINE_SIZE),
                    pAdminEnt);
                if(pAdminEnt != NULL) {
                    UTIL_LogPrintf("[ADMIN] WARNING: Messageex line limit size exceeded: %s\n", sText);
...
static cell playerinfo(AMX* amx, cell* params)
{
    ...
    if(iPlayerIndex > gpGlobals->maxClients) {
        System_Error(UTIL_VarArgs("[ADMIN] ERROR: Request for player info index %d exceeds max player limit %d\n",
                         iPlayerIndex, gpGlobals->maxClients),
            pAdminEnt);
        // amx_RaiseError(amx,AMX_ERR_NATIVE);
        return 0;
    }
        ...
        if(pPlayer) {
            if(iLength == 0 || strnicmp(name, STRING(pPlayer->pev->netname), iLength) == 0) {
                System_Response(UTIL_VarArgs("  %-30.30s  <%4i>    %-17s    %7i  %c\n", STRING(pPlayer->pev->netname),
                                    ...
                                    g_AuthArray[i].iAccess, (g_AuthArray[i].iAccess & ACCESS_IMMUNITY) ? 'y' : 'n'),
                    pAdminEnt);
            }
        }
    }
...
static cell help(AMX* amx, cell* params)
{
    ...
    if(pFileList && length) {
        DEBUG_LOG(1, ("help: Loaded help file '%s' of size %d", help_file, length));
        if(strlen(iszItem) > 2) {
...
static cell check_words(AMX* amx, cell* params)
{
    ...
    while(pLink != NULL) {
        ...
        if(strstr(sBuffer, tWord->sWord) != NULL) {
            DEBUG_LOG(
                1, ("check_words: User '%s' matched swear word '%s'", STRING(pAdminEnt->v.netname), tWord->sWord));
            return 0;
        }
        pLink = pLink->NextLink();
    }
...
static cell amx_get_userIP(AMX* amx, cell* params)
{
    ...
    if(!user_ip(iPlayerIndex, &pcIP, &ulNBOIP))
        return 0;
...
/* Attempt to use the super-duper CS team count function */
    ...
    for(i = 1; i <= gpGlobals->maxClients; i++) {
        ...
        if(IsPlayerValid(pPlayer)) {
            ...
            if(iCS == 1) {
                if(get_player_team(pPlayer) == iTeam)
                    iTeamCount++;
            } else {
...
static cell get_userorigin(AMX* amx, cell* params)
{
    ...
    DEBUG_LOG(2, ("Origin For %s: X: %f Y: %f Z: %f\n", PlayerText, floor(pPlayer->edict()->v.origin.x),
                     floor(pPlayer->edict()->v.origin.y), floor(pPlayer->edict()->v.origin.z)));
...
static cell execclient(AMX* amx, cell* params)
{
    ...
    while(pcStart < pcEOB) {
        ...
        if((pCmd = pat_search_key(eclist, pcStart)) && (pCmd->atAttribute != 1)) {
            System_Response(
                UTIL_VarArgs(const_cast<char*>(get_am_string(0, 0, statstr[5], statstr_table)), pcStart), pAdminEnt);
            return 0;
        } // if
        if(pcEnd++ != pcStart) {
    ...
    if(!stricmp(CmdText, "kill")) {
        if(pPlayer->edict()->v.deadflag != DEAD_NO) {
            System_Response(
                "That player is not currently alive.  The 'kill' command would have no effect.\n", pAdminEnt);
            return 0;
        }
...
static cell godmode(AMX* amx, cell* params)
{
    ...
    if(iGodMode != 0) {
        ...
        UTIL_ClientPrintAll(
            HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(0, 0, statstr[0], statstr_table)),
                               STRING(pPlayer->pev->netname)));
    } else {
        pPlayer->edict()->v.solid = SOLID_SLIDEBOX;
        pPlayer->edict()->v.takedamage = DAMAGE_AIM;
        UTIL_ClientPrintAll(
            HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(0, 0, statstr[1], statstr_table)),
                               STRING(pPlayer->pev->netname)));
    }
    return 1;
}
...
static cell noclip(AMX* amx, cell* params)
{
    ...
    if(iMoveType != 0) {
        ...
        UTIL_ClientPrintAll(
            HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(0, 0, statstr[2], statstr_table)),
                               STRING(pPlayer->pev->netname)));
    } else {
        pPlayer->edict()->v.movetype = MOVETYPE_WALK;
        UTIL_ClientPrintAll(
            HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(0, 0, statstr[3], statstr_table)),
                               STRING(pPlayer->pev->netname)));
    }
...
static cell spawn(AMX* amx, cell* params)
{
    ...
    if(pCheckEntity == NULL) {
        System_Response(
            "[ADMIN] spawn: The entity to be spawned was not cached at map load, so cannot be spawned.\n", pAdminEnt);
        return 0;
    }
    ...
    if(pcExtension != 0 && *(pcExtension + strlen(extension)) == '\0') {
        ...
        if((strlen(pcText) + 2 + strlen(strGameDir)) < 2048) {
            ...
            *pcFilePath = '/';
            ...
            if(pcFilePath == 0)
                return 0; // always wear safety helmets
...
static cell givehealth(AMX* amx, cell* params)
{
    ...
    if(iHealth >= 0) {
        ...
        UTIL_ClientPrintAll(
            HUD_PRINTTALK, UTIL_VarArgs("%s given %i Health\n", STRING(pPlayer->pev->netname), iHealth));
...
AMX_NATIVE_INFO admin_Natives[] = { { "access", access }, { "auth", auth }, { "ban", ban },
    { "censor_words", censor_words }, { "centersay", centersay }, { "centersayex", centersayex },
    { "changelevel", changelevel }, { "cvar_exists", amx_cvar_exists }, { "check_words", check_words },
    { "check_user", check_user_amx }, { "consgreet", consgreet }, { "convert_string", convert_string },
    { "currentmap", currentmap }, { "deletefile", deletefile }, { "directmessage", directmessage }, { "exec", exec },
    { "execclient", execclient }, { "fileexists", fileexists }, { "filesize", filesize }, { "getstrvar", getstrvar },
    { "gettarget", gettarget }, { "getteamcount", getteamcount }, { "getvar", getvar },
    { "get_serverinfo", get_serverinfo }, { "get_timer", get_timer }, { "get_userAuthID", get_user_authid },
    { "get_userindex", get_userindex }, { "get_userinfo", get_userinfo }, { "get_userIP", amx_get_userIP },
    { "get_username", get_username }, { "get_userorigin", get_userorigin }, { "get_userSessionID", get_userSessionID },
    { "get_userWONID", get_userWONID }, { "get_vaultdata", get_vault_str_data },
    { "get_vaultnumdata", get_vault_num_data }, { "glow", glow }, { "godmode", godmode }, { "help", help },
    { "kick", kick }, { "kill_timer", kill_timer }, { "listspawn", listspawn }, { "list_maps", list_maps },
    { "log", log }, { "look_in_dir", look_in_dir }, { "maptime", maptime }, { "maxplayercount", maxplayercount },
    { "menu", menu }, { "message", message }, { "messageex", messageex }, { "movespawn", movespawn },
    { "nextmap", nextmap }, { "noclip", noclip }, { "playercount", playercount }, { "playerinfo", playerinfo },
    { "playsound", playsound }, { "plugin_checkcommand", plugin_checkcommand }, { "plugin_message", plugin_message },
    { "plugin_exec", plugin_exec }, { "plugin_registercmd", plugin_registercmd },
    { "plugin_registerhelp", plugin_registerhelp }, { "plugin_registerinfo", plugin_registerinfo },
    { "pointto", pointto }, { "rainbow", rainbow }, { "readfile", readfile }, { "reload", reload },
    { "removespawn", removespawn }, { "resetfile", resetfile }, { "say", say }, { "selfmessage", selfmessage },
    { "servertime", servertime }, { "setvar", setvar }, { "setstrvar", setstrvar },
    { "set_serverinfo", set_serverinfo }, { "set_timer", set_timer }, { "set_vaultdata", set_vault_str_data },
    { "set_vaultnumdata", set_vault_num_data }, { "slap", slap }, { "slay", slay }, { "spawn", spawn },
    { "speakto", am_speakto }, { "strtonum", strtonum }, { "systemtime", systemtime }, { "teleport", teleport },
    { "timeleft", timeleft }, { "typesay", typesay }, { "unban", unban }, { "userlist", userlist },
    { "valid_map", valid_map }, { "valid_mapex", valid_mapex }, { "version", version }, { "vote", vote_multiple },
    { "vote_allowed", vote_allowed }, { "writefile", writefile }, { "get_userFrags", get_userFrags },
    { "get_userHealth", get_userHealth }, { "get_userTeam", get_userTeam }, { "get_userArmor", get_userArmor },

    { "givehealth", givehealth }, { "takehealth", takehealth },
