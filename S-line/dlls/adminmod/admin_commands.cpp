...
constexpr int print_tty = 3;
constexpr int print_pretty = 4;
...
char* INVALID_TIMER_PARAM = nullptr;
  ...
  if(sUser[0] != '\0') {
	...
	// If we're console, we have full access
	  if ( pAdminEnt== nullptr ) {
		  if ( iAccess < 0 ) return static_cast<int>(-1);
		  return 1;
	  }  // if
	  if ( iAccess < 0 ) return static_cast<ucell>(GetUserAccess(pAdminEnt));
...
static cell ban(AMX *amx, cell *params) {
	...
	constexpr int c_iBanByID = 0; // not used? [APG]RoboCop[CL]
	constexpr int c_iBanByIP = 1;
	constexpr int c_iBanBoth = 2;
  ...
  if ( !bIsIP ) {
	...
	if ( iSessionID > 0 ) {
		util_kick_player( iSessionID, nullptr );
	}
...
static cell centersay(AMX *amx, cell *params) {
  ...
  if (static_cast<int>(CVAR_GET_FLOAT("pretty_say")) != 0) {
	...
	m_textParms.fxTime = params[2] / static_cast<float>(2);
...
static cell centersayex(AMX *amx, cell *params) {
  ...
  CBaseEntity* pPlayer = UTIL_PlayerByIndex(iPlayerIndex);
  if (!IsPlayerValid(pPlayer)) {
	  //int iLineFeed = 0; // not used? [APG]RoboCop[CL]
  ...
  if (static_cast<int>(CVAR_GET_FLOAT("pretty_say")) != 0) {
	...
	//Consider utilizing an explicit type cast to avoid the loss of a fractional part.
	//An example : double A = (double)(X) / Y; //[APG]RoboCop[CL]
	m_textParms.fxTime = params[3]/static_cast<float>(2);
...
static cell changelevel(AMX *amx, cell *params) {
  ...
  if ( iIPause > 0 ) {
	  ...
	  const int iTimer = pEntity->AddTimer(amx,iIPause,0,"ChangeMap",szNextMap,pAdminEnt);
...
static cell amx_cvar_exists(AMX *amx, cell *params) {
  ...
  return (CVAR_GET_POINTER(sCvar) != nullptr) ? 1 : 0;
}
...
static cell convert_string(AMX* amx, cell* params) {
	...
	// Ensure params[1] is a valid pointer
	const char* sHLString = reinterpret_cast<const char*>(static_cast<uintptr_t>(params[1]));
	DEBUG_LOG(4, ("convert_string() string: %p", sHLString));

	const int iLength = params[3];
...
static cell directmessage(AMX *amx, cell *params) {
	...
	} else if ( (pPlayer = get_player_edict(params[2], static_cast<uidt>(params[3]))) == nullptr ) {
...
bool is_registering_command( const char* _pcCommand ) {
	...
	if ( _pcCommand == nullptr || strchr(_pcCommand, ' ') == nullptr ) return false;

	const size_t iCmdLen = strlen( _pcCommand );
...
static cell exec(AMX *amx, cell *params) {
  ...
  char* pcPos = sCommand;
  char* pcNextSpace = nullptr; // not used? [APG]RoboCop[CL]
  int iSemicolons = 0;

  // count semicolons
  while ( ((pcPos = strchr(pcPos, ';')) != nullptr) ) {
	  ++iSemicolons;
	  ++pcPos;
  }  // while
  ...
  if ( (pcPos = strstr(sCommand, "admin_command")) != nullptr ) {
	  ...
	  if ( ! is_registering_command(pcCmnd) ) {
...
static cell get_vault_num_data(AMX *amx, cell *params) {
	...
	if (sKey[0] != '\0') {
		...
		*cStr = static_cast<cell>(strtol(sData, nullptr, 0));
		return 1;
	}
	return 0;
}
...
static cell kick(AMX *amx, cell *params) {
  ...
  CBaseEntity *pPlayer = UTIL_PlayerByIndex(iPlayerIndex);
  if (IsPlayerValid(pPlayer)) {
	util_kick_player( GETPLAYERUSERID(pPlayer->edict()), nullptr );
...
static cell log(AMX *amx, cell *params) {
  ...
  for(int i = 0; i < static_cast<int>(strlen(sText)); i++) {
...
static cell menu(AMX *amx, cell *params) {
  ...
  if ( get_option_cvar_value("amv_enable_beta", "menu", nullptr, 0) != 1 ) {
  ...
  const int iKeys = params[3];
  const int iTime = params[4];
  ...
  CBaseEntity* pPlayer = UTIL_PlayerByIndex(iPlayerIndex); 
  if ( ptAM_botProtection && static_cast<int>(ptAM_botProtection->value) == 1 ) { //Protection agains bots
...
static cell messageex(AMX *amx, cell *params) {
  ...
  if (iType == print_center) {
	  ...
	  m_textParms.b2 = 250;                                                            
	  m_textParms.a2 = 0;   
	  m_textParms.fadeinTime = 0.02f;
	  m_textParms.fadeoutTime = 0;
	  m_textParms.holdTime = 7;
	  m_textParms.fxTime = 0.07f;
	  m_textParms.effect = 2;
	  m_textParms.x = 0.05f;
	  m_textParms.y = 0.7f;
	  m_textParms.channel = 1;
...
static cell playerinfo(AMX *amx, cell *params) {
  ...
  cell* cStr;
	...
	CHECK_AMX_PARAMS(3)
	const int iPlayerIndex = params[1];
  if (iPlayerIndex < 1) {
...
static cell plugin_exec( AMX* amx, cell* params) {
  ...
  if ( !stricmp(sCmd, "admin_help")) {
	  return 0;
  }
  if ( !stricmp(sCmd, "admin_help_hlrat")) {
	  return 0;
  }
  if ( !stricmp(sCmd, "admin_version")) {
	  return 0;
  }

  // execute the command via HandleCommand() if we are using plugins
  if (g_fRunPlugins) {
	  const int iRetVal = HandleCommand(pAdminEnt, sCmd, sData);
...
static cell set_timer(AMX *amx, cell *params) {
  ...
  const int iWait = params[2];
  const int iRepeat = params[3];
  if(iNumParams >= 4) {
  ...
  const int iTimer = pEntity->AddTimer(amx,iWait,iRepeat,sFunction,sParam,pAdminEnt);
  if (iTimer == INVALID_TIMER)  {
...
static cell timeleft(AMX *amx, cell *params) {
  ...
  if ( g_pflTimeLimit == nullptr ) {
	  flTimeLimit = CVAR_GET_FLOAT("mp_timelimit") * 60.0f; // map timelimit in sec
  } else {
	  flTimeLimit = *g_pflTimeLimit;  // Counterstrike's timelimit value
  }  // if-else

  if (iNumParams == 0 || params[1] == 1) {
	  const int timeleft = static_cast<int>(flTimeLimit - gpGlobals->time);
	  const int minleft = timeleft / 60;
	  const int secleft = timeleft % 60;
	System_Response(UTIL_VarArgs( "[ADMIN] Time left on map: %02d:%02d.\n",minleft, secleft), pAdminEnt);
  }  // if
  return static_cast<cell>(flTimeLimit - gpGlobals->time);
}           
...
static cell userlist(AMX *amx, cell *params) {
  ...
  for ( int i = 1; i <= gpGlobals->maxClients; i++ ) {
	...
	if (pPlayer) {
	  if(iLength==0 || _strnicmp(name, STRING(pPlayer->pev->netname), iLength)==0) {
...
static cell getvar(AMX *amx, cell *params) {
  ...
  char *iszItem=name; // not used? [APG]RoboCop[CL]

  const int value = static_cast<int>(CVAR_GET_FLOAT(name));
  return value;
}
...
static cell setvar(AMX *amx, cell *params) {
  ...
  CVAR_SET_FLOAT(name,static_cast<float>(params[2]));
  return 1;
}
...
static cell censor_words(AMX *amx, cell *params) {
  ...
  for (i = 0; i < static_cast<int>(strlen(sBuffer)); i++)
  ...
  while (pLink != nullptr) {
	  ...
	  const int wordLength = static_cast<int>(strlen(tWord->sWord)); // Store the length of the word
	  while (sWord != nullptr) {
		  const int iOffset = (sWord - &sBuffer[0]) - 1;
		  for (i = 1; i <= wordLength; i++) { // Use the stored length
			  sReturnBuffer[iOffset + i] = '*';
		  }
		  sWord = strstr(sWord + 1, tWord->sWord);
		  iRetVal = 0;
	  }
...
static cell check_words(AMX *amx, cell *params) {
  ...
  for (int i = 0; i < static_cast<int>(strlen(sBuffer)); i++)
	sBuffer[i] = tolower(sBuffer[i]);
  
  CLinkItem<word_struct>* pLink = m_pWordList->FirstLink();
  while (pLink != nullptr) {
	word_struct* tWord = pLink->Data();
	if (strstr(sBuffer, tWord->sWord) != nullptr ) {
...
static cell amx_get_userIP( AMX *amx, cell *params ) {
	...
	if (iNumParams >= 4 && params[4] > amx->hea) {
	  amx_GetAddr(amx,params[4],&cParam);
	  *cParam = g_AuthArray[iPlayerIndex].iPort;
	}

	return static_cast<cell>(ulNBOIP);
...
static cell get_username(AMX *amx, cell *params) {
  ...
  if(params[3] < static_cast<int>(strlen(STRING(pPlayer->pev->netname)))) {
	amx_RaiseError(amx,AMX_ERR_NATIVE);
	return 0;
  }
...
static cell get_userSessionID(AMX *amx, cell *params) {
  ...
  amx_GetAddr(amx,params[2],&Param);
  *Param = GETPLAYERUSERID(pPlayer->edict());
  return 1;
}
...
static cell get_userWONID(AMX *amx, cell *params) {
  ...
  amx_GetAddr(amx,params[2],&Param);
  *Param = static_cast<cell>(GETPLAYERWONID(pPlayer->edict()));
  return 1;
}
...
static cell get_user_authid(AMX *amx, cell *params) {
  ...
  amx_GetAddr(amx,params[2],&Param);
  if(params[3] < static_cast<int>(strlen(GETPLAYERAUTHID(pPlayer->edict())))) {
	amx_RaiseError(amx,AMX_ERR_NATIVE);
	return 0;
  }
...
static cell execclient(AMX *amx, cell *params) {
  ...
  if (static_cast<int>(CVAR_GET_FLOAT("allow_client_exec"))!=1) {
  ...
  while ( pcStart < pcEOB ) {
	  ...
	  if ( (pCmd = pat_search_key(eclist, pcStart)) && (pCmd->atAttribute != 1) ) {
		  System_Response( UTIL_VarArgs(const_cast<char*>(get_am_string(nullptr,0,statstr[5],statstr_table)), pcStart), pAdminEnt );
		  return 0;
	  } // if
	  if ( pcEnd++ != pcStart ) {
  ...
  if (!stricmp(CmdText,"kill")) {
	...
	if (static_cast<int>(CVAR_GET_FLOAT("admin_fx")) != 0) {
		const Vector& origin = pPlayer->edict()->v.origin;
...
static cell teleport(AMX *amx, cell *params) {
  ...
  pPlayer->edict()->v.origin.x = static_cast<float>(params[2]);
  pPlayer->edict()->v.origin.y = static_cast<float>(params[3]);
  pPlayer->edict()->v.origin.z = static_cast<float>(params[4]);
...
static cell fileexists(AMX *amx, cell *params) {
  ...
  if ( get_file_path(acFilePath, acFilename, PATH_MAX, nullptr) > 0 ) {
	  FILE * fFile;
...
static cell readfile(AMX *amx, cell *params) {
	...
	constexpr int c_LineLen = LARGE_BUF_SIZE;
  ...
  if ( get_file_path(acFilePath, acFilename, PATH_MAX, "file_access_read") > 0 ) {
	...
	if ( strlen(acLine) >= 0 ) {
	  amx_GetAddr( amx, params[2], &cptr );
	  if( params[4] < static_cast<cell>(strlen(acLine)) ) {
		  // amx_RaiseError( amx, AMX_ERR_NATIVE );
		return 0;
	  }  // if
...
static cell writefile(AMX *amx, cell *params) {
  ...
  if ( get_file_path(acFilePath, acFilename, PATH_MAX, "file_access_write") > 0 ) {
	  ...
	  if( (fFile = fopen(acFilePath,"a+")) == nullptr ) {
	...
	return 0;
	  ...
	  */
	  ...
	  if( (tmp_file = fopen(acTempFilePath,"w+")) == nullptr ) {
	...
	i++;
	if ( fgets(acLine, c_LineLen, fFile) != nullptr) {
	  if( i == params[3] )
	...
	fprintf( tmp_file, "%s%s", acData, pcEOL);
	  ...
	  fclose( fFile );
	  if( (fFile = fopen(acFilePath,"w")) == nullptr ) {
...
static cell deletefile(AMX *amx, cell *params) {
  ...
  int iLen;
  const int iNumParams = params[0]/sizeof(cell);
  int iRetVal = 0; // not used? [APG]RoboCop[CL]
  char acFilename[BUF_SIZE];
...
static cell get_userinfo(AMX *amx, cell *params) {
  ...
  if(params[4] < static_cast<int>(strlen(value))) {
	amx_RaiseError(amx,AMX_ERR_NATIVE);
	return 0;
  }
...
static cell get_serverinfo(AMX *amx, cell *params) {
  ...
  if(params[3] < static_cast<int>(strlen(value))) {
	amx_RaiseError(amx,AMX_ERR_NATIVE);
	return 0;
  }
  amx_SetString(cptr,value,0);
...
static cell set_serverinfo(AMX *amx, cell *params) {
  ...
  int len;
  const int iNumParams = params[0]/sizeof(cell);
  int PlayerIndex = 0; // not used? [APG]RoboCop[CL]
...
static cell slap(AMX *amx, cell *params) {
  ...
  */
  ...
  /* Send 'em sprawling */
  Vector& velocity = pPlayer->edict()->v.velocity;
  velocity.x += floor(RANDOM_FLOAT(-800, 800));
  velocity.y += floor(RANDOM_FLOAT(-800, 800));
  if (velocity.z >= 200) {
	  velocity.z -= floor(RANDOM_FLOAT(100, 200));
  }
  else {
	  velocity.z += floor(RANDOM_FLOAT(100, 200));
  }
...
static cell godmode(AMX *amx, cell *params) {
  ...
  const int iGodMode = params[2];
  if(iGodMode!=0) {
	pPlayer->edict()->v.solid = SOLID_NOT;
	pPlayer->edict()->v.takedamage = DAMAGE_NO;
  UTIL_ClientPrintAll( HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(nullptr,0,statstr[0],statstr_table)),STRING(pPlayer->pev->netname))); 
  } else {
	pPlayer->edict()->v.solid = SOLID_SLIDEBOX;
	pPlayer->edict()->v.takedamage = DAMAGE_AIM;
  UTIL_ClientPrintAll( HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(nullptr,0,statstr[1],statstr_table)),STRING(pPlayer->pev->netname))); 
  }
  return 1;
}
...
static cell noclip(AMX *amx, cell *params) {
  ...
  const int iMoveType = params[2];
  if(iMoveType!=0) {
	pPlayer->edict()->v.movetype = MOVETYPE_NOCLIP;
  UTIL_ClientPrintAll( HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(nullptr,0,statstr[2],statstr_table)),STRING(pPlayer->pev->netname))); 
  } else {
	pPlayer->edict()->v.movetype = MOVETYPE_WALK;
  UTIL_ClientPrintAll( HUD_PRINTTALK, UTIL_VarArgs(const_cast<char*>(get_am_string(nullptr,0,statstr[3],statstr_table)),STRING(pPlayer->pev->netname))); 
  }
...
static cell spawn(AMX *amx, cell *params) {
  ...
  pEntity->edict()->v.origin.x = static_cast<float>(params[2]);
  pEntity->edict()->v.origin.y = static_cast<float>(params[3]);
  pEntity->edict()->v.origin.z = static_cast<float>(params[4]);
  pEntity->edict()->v.angles.x = static_cast<float>(params[5]);
  pEntity->edict()->v.angles.y = static_cast<float>(params[6]);
  pEntity->edict()->v.angles.z = static_cast<float>(params[7]);
  GameDispatchSpawn( pEntity->edict() );

	const int iIdentity = AddSpawnEntity(szClassname, pEntity);
  if(iIdentity==0) {
...
static cell systemtime(AMX *amx, cell *params) {
  return static_cast<cell>(time(nullptr));
}
...
static cell maptime(AMX *amx, cell *params) {
  ...
  if ( iNumParams<2) {
	...
	const float flTimeLimit = CVAR_GET_FLOAT("mp_timelimit") * 60; // map timelimit in sec

  // if we should return the time left
  if ( params[1] == 1 ) {
	  const float timeleft = flTimeLimit - gpGlobals->time;

	if ( params[2] == 1 ) {
		const int minleft = static_cast<int>(timeleft) / 60;
		const int secleft = static_cast<int>(timeleft) % 60;
	  System_Response(UTIL_VarArgs( "timeleft on map: %d:%d\n",minleft, secleft),pAdminEnt);
	}  // if

	return static_cast<cell>(timeleft);
  }  else {
	// we return the map time

	if ( params[2] == 1 ) {
		const int min = static_cast<int>(gpGlobals->time / 60);
		const int sec = static_cast<int>(gpGlobals->time) % 60;
	  System_Response(UTIL_VarArgs( "time into map: %d:%d\n",min, sec),pAdminEnt);
	}  // if

	return static_cast<cell>(gpGlobals->time);
  }  // if-else
...
static cell servertime(AMX *amx, cell *params) {
  ...
  if (iNumParams  < 1) {
	return static_cast<cell>(myTime);
  }  // if
...
static cell consgreet(AMX *amx, cell *params) {
  ...
  char* pcMsgFile = nullptr;        // the message read from a file
  char strGameDir[2048];  
  (*g_engfuncs.pfnGetGameDir)(strGameDir); 

  // check if the string provided ends with ".txt"
  if ( pcExtension != nullptr && *(pcExtension + strlen(extension)) == '\0' ) {
...
static cell look_in_dir(AMX *amx, cell *params) {
  ...
  const long hnd = _findfirst(game_dir, &filestruct);
...
static cell gettarget(AMX *amx, cell *params) {
  ...
  if (tr.flFraction != 1.0 && !FNullEnt( tr.pHit ) ) {
	  ...
	  if ( params[3] > 0 ) {
		  if ( params[3] > static_cast<int>(strlen(STRING(pEntity->pev->netname))) ) {
...
static cell pointto(AMX *amx, cell *params) {
  ...
  Vector vecEnd = vecSrc + (gpGlobals->v_forward * params[1]); 
...
static cell get_userFrags(AMX *amx, cell *params) {
  ...
  amx_GetAddr(amx,params[2],&Param);
  *Param = static_cast<cell>(pPlayer->edict()->v.frags);
  return 1;
}
...
static cell get_userHealth(AMX *amx, cell *params) {
  ...
  amx_GetAddr(amx,params[2],&Param);
  *Param = static_cast<cell>(pPlayer->edict()->v.health);
  return 1;
}
...
static cell get_userArmor(AMX *amx, cell *params) {
  ...
  amx_GetAddr(amx,params[2],&Param);
  *Param = static_cast<cell>(pPlayer->edict()->v.armorvalue);
  return 1;
}
// end of CD addins
