...
//#include <cbase.h> // No longer required? [APG]RoboCop[CL]
...
//#include <gamerules.h> // No longer required? [APG]RoboCop[CL]
...
//#include <nodes.h>
//#include <plane.h>
//#include <player.h> // No longer required? [APG]RoboCop[CL]
//
...
// the CTimer class
//#include "timer.h" // No longer required? [APG]RoboCop[CL]
...
long int get_option_cvar_value(const char* CvarName,
    const char* Option,
    long int NumDefault,
    char* ReturnString,
    size_t Len,
    const char* StringDefault = NULL);
...
{
    return ((pPlayer) && (!FNullEnt(pPlayer->pev)) && (GETPLAYERUSERID(pPlayer->edict()) > 0) &&
...
{
    return ((pPlayer) && (!FNullEnt(pPlayer)) && (GETPLAYERUSERID(pPlayer) > 0) &&
               (!FStrEq(STRING(pPlayer->v.netname), ""))) ?
        TRUE :
        FALSE;
}
...
inline bool cvar_string_value_is_set(const char* _pcCvarValueString, bool _bAllowEmptyString = false)
{
    ...
    if(_pcCvarValueString == NULL)
        return false;

    // if the string is empty and we do not allow this case, return false
    if(*_pcCvarValueString == '\0' && !_bAllowEmptyString)
        return false;

    // if the string equals "0", return false
    if(*_pcCvarValueString == '0' && *(_pcCvarValueString + 1) == '\0')
        return false;
...
inline const char* get_cvar_string_value(const char* _pcCvarName, bool _bAllowEmptyString = false)
{
    ...
    // get the CVAR string
    const char* pcCvarValue = CVAR_GET_STRING(_pcCvarName);
    if(cvar_string_value_is_set(pcCvarValue, _bAllowEmptyString))
        return pcCvarValue;
    else
        return NULL;
}
...
inline const char* get_cvar_file_value(const char* _pcCvarName)
{
    ...
    // get the CVAR string
    const char* pcCvarValue = CVAR_GET_STRING(_pcCvarName);
    if(cvar_file_value_is_set(pcCvarValue))
        return pcCvarValue;
    else
        return NULL;
}
