...
cvar_t mysql_tags_sql = { "mysql_tags_sql", "SELECT pass,access FROM %s where '%s' REGEXP nick or nick='%s'",
    FCVAR_EXTDLL };
...
int AM_ClientCommand(edict_t* pEntity)
{
    ...
    commandline[0] = 0;
    if(CMD_ARGS())
        strncpy(commandline, CMD_ARGS(), 255);
    ...
    if(FStrEq(pcmd, "admin_login") || FStrEq(pcmd, "admin_password")) {
        ...
        if(cvar_file_is_set("admin_plugin_file"))
            ...
            System_Response("Admin Mod plugins are disabled.\n", pEntity);
    ...
    if(g_fRunPlugins) {
        ...
        if(program_file == NULL || FStrEq(program_file, "0") || !g_fRunScripts) {
            ...
            if(iError != AMX_ERR_NONE) {
                ...
                if(pEntity != NULL)
                    iError = amx_Exec(&amx, &cReturn, iIndex, 5, pcmd, CMD_ARGS(), STRING(pEntity->v.netname),
                        GETPLAYERUSERID(pEntity), GETPLAYERWONID(pEntity));
                else
...
#define CLIENT_CONNECT_RETURN(a)   \
    bAM_ClientConnectRetval = (a); \
    return a

BOOL AM_ClientConnect(edict_t* pEntity,
    const char* pszName,
    ...
    bool bForce)
{
    ...
    if(pszAddress) {
        if(0 == strcmp(pszAddress, "loopback"))
            pszAddress = "127.0.0.1:27005";
    ...
    if(!oaiAuthID.is_loopid() && (int)CVAR_GET_FLOAT("amv_private_server") != 0) {
        ...
        if(!(GetUserRecord(sName, oaiAuthID, sIP, g_AuthArray[iIndex].sPassword, 0)) && !(IsIPReserved(sIP))) {
        ...
        if(IsIPReserved(sIP) || ((GetUserAccess(pEntity) & ACCESS_RESERVE_SPOT) == ACCESS_RESERVE_SPOT)) {
            ...
            if(sReserveMsg == NULL) {
                snprintf(szRejectReason, 128, "\n[ADMIN] %d of the %d slots on this server are reserved,\n[ADMIN] and "
                                              "no unreserved slots are left. Try again later.\n",
...
void AM_ClientStart(edict_t* pEntity)
{
    ...
    */
    ...
    if(passwd_field == NULL) { // Default value "0", i.e. config file not read.
        UTIL_LogPrintf("\n[ADMIN] ERROR: ********************************************************\nThe configuration "
                       "file for Admin Mod (default: adminmod.cfg) could not be read.\nMake sure that the Admin Mod "
                       "configuration file is executed from server.cfg\nwhen you use Admin Mod.\nGo to "
                       "http://www.adminmod.org/manual/ for more details.\n");
#ifdef WIN32
        MessageBox(NULL, "[ADMIN] ERROR:\n\nThe configuration file for Admin Mod could not be read.\nMake sure that "
                         "the Admin Mod configuration file (default: adminmod.cfg) is executed from server.cfg when "
        ...
        UTIL_LogPrintf("[ADMIN] WARNING: You have not set a password_field value. This is needed for your admins to "
                       "securely authenticate with the server.\n");

    } else if(passwd_field[0] != '_') { // Not starting with an underscore.
        UTIL_LogPrintf("\n[ADMIN] ERROR: *********************************************************\nFor your own "
                       "safety you are required to set the password_field cvar to a value\nwhich is prefixed with an "
                       "underscore. This is to protect your admins' passwords\nfrom being distributed to other "
                       "clients.\n\nChange the value for password_field in your adminmod.cfg file\nfrom '%s' to "
                       "'_%s'.\nGo to http://www.adminmod.org/manual/ for more details.\n",
            passwd_field, passwd_field);
#ifdef WIN32
        char acErrMsg[500];
        snprintf(acErrMsg, 500, "[ADMIN] ERROR:\n\nFor your own safety you are required to set the password_field cvar "
                                "to a value which is prefixed with an underscore. This is to protect your admins' "
                                "passwords from being distributed to other clients.\n\nChange the value for "
                                "password_field in your adminmod.cfg file from '%s' to '_%s'.\nGo to "
                                "http://www.adminmod.org/manual/ for more details.\n",
            passwd_field, passwd_field);
    ...
    if(sPluginFile == NULL) {
        ...
        if(program_file != NULL) {
            /*TBR This isn't used anymore, we allow not using any plugins now
              if( program_file == NULL ) {
          #ifdef WIN32
                MessageBox(NULL,"[ADMIN] ERROR:\n\nYou must define \"admin_plugin_file\" in your adminmod.cfg file
          before you can use Admin Mod.\nGo to http://www.adminmod.org/help/online/ for more details.\n","ERROR",MB_OK |
          MB_ICONSTOP | MB_DEFBUTTON1 | MB_SYSTEMMODAL);
          #else
                fprintf(stderr,"[ADMIN] ERROR: **********************************\n\nYou must define
          \"admin_plugin_file\" in your adminmod.cfg file before you can use Admin Mod.\nGo to
          http://www.adminmod.org/help/online/ for more details.\n");
          #endif
                am_exit(1);
              }
            */
...
int AM_ClientUserInfoChanged(edict_t* pEntity, char* infobuffer, bool bForce)
{
    ...
    if(g_AuthArray[iIndex].state == auth_struct::dirty || pEntity->v.netname == 0 ||
        FStrEq(STRING(pEntity->v.netname), "")) {
        DEVEL_LOG(
            2, ("AM_ClientUserInfoChanged() called on unintialized player (state==%d), (netname==%i). Call ignored",
                   g_AuthArray[iIndex].state, pEntity->v.netname));
        return RESULT_CONTINUE;
    }
    ...
    if(!FStrEq(sName, STRING(pEntity->v.netname)) || bForce) {
        ...
        if(!oaiAuthID.is_loopid() && (int)CVAR_GET_FLOAT("amv_private_server") != 0) {
            ...
            if(!(GetUserRecord(sName, oaiAuthID, g_AuthArray[iIndex].sIP, g_AuthArray[iIndex].sPassword, 0)) &&
                ...
                if(sRsvdRejectMsg == NULL) {
                    am_strncpy(
                        acKickMsg, "[ADMIN] Only registered nicknames are allowed on this server.", c_KICK_MSG_LEN);
                } else {
        ...
        if(0 == iKick) {
            ...
            if(IsNameReserved(sName, oaiAuthID, 0, &tUser)) {
                if(!oaiAuthID.is_loopid() &&
                    ...
                    if(sNickRejectMsg == NULL) {
                        CLIENT_PRINTF(
                            pEntity, print_console, "[ADMIN] Your name or WON ID is reserved on this server.\n");
                        am_strncpy(
                            acKickMsg, "[ADMIN] Your name or WON ID is reserved on this server.", c_KICK_MSG_LEN);
                    } else {
                        CLIENT_PRINTF(pEntity, print_console, UTIL_VarArgs("%s\n", sNickRejectMsg));
    ...
    if(g_fRunPlugins) {
        ...
        if(program_file == NULL || FStrEq(program_file, "0")) {
            ...
            if(iError != AMX_ERR_NONE) {
                ...
                iError = amx_Exec(&amx, &cReturn, iIndex, 4, STRING(pEntity->v.netname), sName,
                    GETPLAYERUSERID(pEntity), GETPLAYERAUTHID(pEntity));
...
int AM_GetGameDescription(const char* _pcDescription)
{
    ...
    if((iFreeSlots != s_iFreeSlots) || (iPlayerCnt != s_iPlayerCnt) || (iVisMaxPlayers != s_iVisMaxPlayers) ||
        (iHideSlots != s_iHideSlots) || (iResSlots != s_iResSlots) || (iResType != s_iResType)) {
...
int AM_PlayerPreThink(edict_t* pEntity)
{
    ...
    if(g_SpectatorCheatAction == 1) {
        if((pEntity->v.iuser1 == 3) && ((pEntity->v.team != 0) || (pEntity->v.playerclass != 0)) &&
            !pEntity->v.deadflag) {
...
int AM_StartFrame(void)
{
    ...
    if(s_uiFCount % 500 == 0) { // execute every X frames
        ...
        if(!g_ovcPendingPlayers.empty()) {
            QUEUEING_DEBUGF(
                ("|---> (Start Frame) Checking pending player queue. Queue length: %d\n", g_ovcPendingPlayers.size()));
            ...
            if(AMAuthId::is_pending(pcAuthId)) {
                ...
                BOOL retval =
                    AM_ClientConnect(pePPlayer->pEntity, pePPlayer->acName, pePPlayer->acIPPort, acRejectMsg, true);
                ...
                if(retval == FALSE) {
                    ...
                    QUEUEING_DEBUGF(
                        ("|----> Call AM_ClientUserInfoChanged( %p, infobuf, true )\n", pePPlayer->pEntity));
                    AM_ClientUserInfoChanged(
                        pePPlayer->pEntity, g_engfuncs.pfnGetInfoKeyBuffer(pePPlayer->pEntity), true);
                }

                QUEUEING_DEBUGF(("|---> Player Auth Id  %s dequeued -- (%d queued)  (StartFrame %lu)\n", pcAuthId,
                    g_ovcPendingPlayers.size(), s_uiFCount));
    ...
    */
    ...
    if((s_bInitCheck || s_bMECheck) && s_uiFCount > 50) {
        ...
        if(s_bInitCheck) {
            ...
            /*TBR: This is not used anymore, we now allow to not use any plugins.
                                    ...
                                    if ( !cvar_file_is_set("admin_plugin_file") ) {
                                            ...
                                            if( !cvar_file_is_set("script_file") ) {
                                                    ...
                                                    // If neither is set we probably have not yet read the adminmod.cfg
            file.
                                                    // Try to read it from a default location and return so that this
            check is run again
                                                    if ( !s_bConfigRead ) {
                                                            char cfgfile[PATH_MAX];
                                                            int pos;
                                                            snprintf( cfgfile, PATH_MAX-1, "exec %s/adminmod.cfg",
            CVAR_GET_STRING("amv_default_config_dir") );
            ...
            ********************************************************\n\nYou must define \"admin_plugin_file\" in
            your\nadminmod.cfg file before you can use Admin Mod.\nGo to http://www.adminmod.org/help/online/ for more
            details.\n");
            #ifdef WIN32
                                                    MessageBox(NULL,"[ADMIN] ERROR:\n\nYou must define
            \"admin_plugin_file\" in your adminmod.cfg file before you can use Admin Mod.\nGo to
            http://www.adminmod.org/help/online/ for more details.\n","ERROR",MB_OK | MB_ICONSTOP | MB_DEFBUTTON1 |
            MB_SYSTEMMODAL);
            #else
                                                    fprintf(stderr,"[ADMIN] ERROR:
            ********************************************************\n\nYou must define \"admin_plugin_file\" in
            ...
            */
            ...
            if(passwd_field == NULL) {
                ...
                UTIL_LogPrintf("\n[ADMIN] ERROR: ********************************************************\nThe "
                               "configuration file for Admin Mod (default: adminmod.cfg) could not be read.\nMake sure "
                               "that the Admin Mod configuration file is executed from server.cfg\nwhen you use Admin "
                               "Mod.\nGo to http://www.adminmod.org/manual/ for more details.\n");
#ifdef WIN32
                MessageBox(NULL, "[ADMIN] ERROR:\n\nYThe configuration file for Admin Mod could not be read.\nMake "
                                 "sure that the Admin Mod configuration file (default: adminmod.cfg) is executed from "
                                 "server.cfg when you use Admin Mod.\nGo to http://www.adminmod.org/manual/ for more "
                                 "details.\n",
                    "ERROR", MB_OK | MB_ICONSTOP | MB_DEFBUTTON1 | MB_SYSTEMMODAL);
#else
                fprintf(stderr, "\n[ADMIN] ERROR: ********************************************************\nThe "
                                "configuration file for Admin Mod (default: adminmod.cfg) could not be read.\nMake "
                                "sure that the Admin Mod configuration file is executed from server.cfg\nwhen you use "
                                "Admin Mod.\nGo to http://www.adminmod.org/manual/ for more details.\n");
#endif
                am_exit(1);

            } else if(passwd_field[0] == '\0') { // Empty password_field.
                UTIL_LogPrintf("[ADMIN] WARNING: You have not set a password_field value. This is needed for your "
                               "admins to securely authenticate with the server.\n");

            } else if(passwd_field[0] != '_') { // Not starting with an underscore.
                UTIL_LogPrintf("[ADMIN] ERROR: *********************************************************\nFor your own "
                               "safety you are required to set the password_field cvar to a value\nwhich is prefixed "
                               "with an underscore. This is to protect your admins' passwords\nfrom being distributed "
                               "to other clients.\n\nChange the value for password_field in your adminmod.cfg "
                               "file\nfrom '%s' to '_%s'.\nGo to http://www.adminmod.org/manual/ for more details.\n",
                    passwd_field, passwd_field);
#ifdef WIN32
                char acErrMsg[500];
                snprintf(acErrMsg, 500,
                    "[ADMIN] ERROR:\n\nFor your own safety you are required to set the password_field cvar to a value "
                    "which is prefixed with an underscore. This is to protect your admins' passwords from being "
...
#else
                fprintf(stderr, "[ADMIN] ERROR: *********************************************************\nFor your "
                                "own safety you are required to set the password_field cvar to a value\nwhich is "
                                "prefixed with an underscore. This is to protect your admins' passwords\nfrom being "
                                "distributed to other clients.\n\nChange the value for password_field in your "
                                "adminmod.cfg file\nfrom '%s' to '_%s'.\nGo to http://www.adminmod.org/manual/ for "
                                "more details.\n",
                    passwd_field, passwd_field);
#endif
                am_exit(1);
...
void KickHighestPinger(const char* pszName, char* ip, edict_t* pEntity)
{
    ...
    for(int i = 1; i <= gpGlobals->maxClients; i++) {
        ...
        if(iPing > iMaxPing) {
            ...
            if(!(IsIPReserved(g_AuthArray[i].sIP) || ((iAccess & ACCESS_RESERVE_SPOT) == ACCESS_RESERVE_SPOT) ||
    ...
    if(iPlayerIndex != 0) {
        ...
        if(sReserveKickMsg == NULL) {
            sReserveKickMsg = "[ADMIN] The Server Admin has enabled reserved slots. You have been kicked due to "
                              "another player taking a reserved slot.";
        }
...
void* LoadScript(AMX* amx, const char* filename)
{
    ...
    // return NULL; //error: Resource leak: fp [APG]RoboCop[CL]
}
