...
#define DOUBLE_FAIL_STACK(fail_stack)                                                                                \
    ((fail_stack).size > (unsigned)(re_max_failures * MAX_FAILURE_ITEMS) ?                                           \
            0 :                                                                                                      \
            ((fail_stack).stack = (fail_stack_elt_t*)REGEX_REALLOCATE_STACK((fail_stack).stack,                      \
                 (fail_stack).size * sizeof(fail_stack_elt_t), ((fail_stack).size << 1) * sizeof(fail_stack_elt_t)), \
                                                                                                                     \
            ...
            for(;;) {
                ...
                /* See if we're at the beginning of a possible character
                   class.  */
                    ...
                    for(;;) {
                        PATFETCH(c);
                        if((c == ':' && *p == ']') || p == pend || c1 == CHAR_CLASS_MAX_LENGTH)
                            break;
                        str[c1++] = c;
                    }
                    str[c1] = '\0';
...
static boolean group_match_null_string_p _RE_ARGS(
