...
#else
// typedef long int32_t; // Conflicting with HLSDK 2015? [APG]RoboCop[CL]
...
public:
    ...
    AMAuthId(uint32_t w)
        ...
        , m_uiWonid(w)
        , m_uiAuthid64(nsAuthid::INVAL_ID_L){};
    AMAuthId(uint32_t w, uint64_t s)
        : m_bIdIsSet(true)
        , m_bIsWonid(false)
        , m_cAuthidType(undef)
        , m_uiWonid(w)
        , m_uiAuthid64(s){};
    AMAuthId(uint32_t w, uint32_t s1, uint32_t s2)
        : m_bIdIsSet(true)
        , m_bIsWonid(false)
        , m_cAuthidType(undef)
        , m_uiWonid(w)
    ...
    AMAuthId(const unsigned char* _pcID)
    {
        f_parse_id((const char*)_pcID);
    };
    AMAuthId(const char* _pcID)
    {
        f_parse_id(_pcID);
    };
    ...
    AMAuthId& operator=(const unsigned char* _pcID)
    {
        f_parse_id((const char*)_pcID);
        return *this;
    };
    AMAuthId& operator=(const char* _pcID)
    {
        f_parse_id(_pcID);
        return *this;
    };
    ...
    operator int() const
    {
        return static_cast<int>(m_uiWonid);
    };
    ...
    {
        return static_cast<long int>(m_uiWonid);
    };
    ...
    operator bool() const
    {
        ...
        if(m_bIsWonid)
            ...
            return (m_uiWonid != 0 && m_uiAuthid64 != 0 && m_cAuthidType != undef);
    }
    ...
    uint32_t wonid(void) const
    {
        return m_uiWonid;
    }
    uint32_t authid_a(void) const
    {
        return m_uiWonid;
    }
    uint64_t authid_b(void) const
    {
        return m_uiAuthid64;
    }

    uint32_t authid_x(void) const
    {
        return m_uiWonid;
    }
    uint32_t authid_y(void) const
    {
        return m_uiAuthid32[nsAuthid::HIGH];
    }
    uint32_t authid_z(void) const
    {
        return m_uiAuthid32[nsAuthid::LOW];
    }

    bool is_wonid(void) const
    {
        return (m_bIdIsSet && m_bIsWonid);
    };
    bool is_authid(void) const
    {
        return (m_bIdIsSet && !m_bIsWonid);
    };
    bool is_steamid(void) const
    {
        return (m_bIdIsSet && !m_bIsWonid && (m_cAuthidType == steam || m_cAuthidType == steam2));
    };
    bool is_valveid(void) const
    {
        return (m_bIdIsSet && !m_bIsWonid && (m_cAuthidType == valve || m_cAuthidType == valve2));
    };
    ...
    bool is_botid(void) const
    {
        return (m_bIsWonid && m_uiWonid == nsAuthid::BOT_ID);
    };
    bool is_lanid(void) const
    {
        return (m_bIsWonid && m_uiWonid == nsAuthid::LAN_ID);
    };
    bool is_loopid(void) const
    {
        return (!m_bIsWonid && m_uiWonid == nsAuthid::LOOP_ID);
    };

    // utility functions
    static bool is_authid(const char* _pcID)
    {
        if(_pcID == NULL || *_pcID == '\0')
            return false;
        return (((_pcID[0] == 'S' && _pcID[1] == 'T' && _pcID[2] == 'E' && _pcID[3] == 'A' && _pcID[4] == 'M') ||
                    (_pcID[0] == 'V' && _pcID[1] == 'A' && _pcID[2] == 'L' && _pcID[3] == 'V' && _pcID[4] == 'E')) &&
    ...
    static bool is_valveid(const char* _pcID)
    {
        if(_pcID == NULL || *_pcID == '\0')
            return false;
        return (_pcID[0] == 'V' && _pcID[1] == 'A' && _pcID[2] == 'L' && _pcID[3] == 'V' && _pcID[4] == 'E' &&
            _pcID[5] == '_' && _pcID[6] >= 0x30 && _pcID[6] <= 0x39);
    };
    ...
    static bool is_lanid(const char* _pcID)
    {
        ...
        const int* pcI = (const int*)_pcID;
        return (((*pcI == VALV && *(pcI + 1) == E_ID) || (*pcI == STEA && *(pcI + 1) == M_ID)) && (*(pcI + 2) == uLAN));
    };
...
inline void AMAuthId::f_assemble_string(void) const
{
    ...
    if(!m_bIdIsSet) {
        ...
        if(m_uiWonid == nsAuthid::LOOP_ID && m_uiAuthid64 == nsAuthid::INVAL_ID_L) {
            ...
            snprintf(m_acString, nsAuthid::STATIC_LEN, "STEAM_%u:%lu:%lu", m_uiWonid, m_uiAuthid32[nsAuthid::HIGH],
                m_uiAuthid32[nsAuthid::LOW]);
            ...
            snprintf(m_acString, nsAuthid::STATIC_LEN, "VALVE_%u:%lu:%lu", m_uiWonid, m_uiAuthid32[nsAuthid::HIGH],
                m_uiAuthid32[nsAuthid::LOW]);
        } else {
...
inline bool operator==(const AMAuthId& lhs, const AMAuthId& rhs)
{
    ...
    if(!lhs.m_bIdIsSet || !rhs.m_bIdIsSet)
        return false;
    if(lhs.m_bIsWonid) {
        if(!rhs.m_bIsWonid)
            return false;
        else
            return (lhs.m_uiWonid == rhs.m_uiWonid);
    } else {
        if(rhs.m_bIsWonid)
            return false;
        else
            return ((lhs.m_uiWonid == rhs.m_uiWonid) && (lhs.m_uiAuthid64 == rhs.m_uiAuthid64) &&
                AMAuthId::same_id_type_class(lhs, rhs));
    } // if
}
...
inline bool operator==(const uint32_t lhs, const AMAuthId& rhs)
{
    ...
    if(!rhs.m_bIdIsSet || !rhs.m_bIsWonid)
        return false;
    else
        return (lhs == rhs.m_uiWonid);
}

inline bool operator==(const AMAuthId& lhs, const uint32_t rhs)
{

    if(!lhs.m_bIdIsSet || !lhs.m_bIsWonid)
        return false;
    else
        return (lhs.m_uiWonid == rhs);
}
...
inline bool operator==(const int32_t lhs, const AMAuthId& rhs)
{
    ...
    if(!rhs.m_bIdIsSet || !rhs.m_bIsWonid)
        ...
        return (static_cast<uint32_t>(lhs) == rhs.m_uiWonid);
}
...
inline bool operator==(const AMAuthId& lhs, const int32_t rhs)
{
    ...
    if(!lhs.m_bIdIsSet || !lhs.m_bIsWonid)
        ...
        return (lhs.m_uiWonid == static_cast<uint32_t>(rhs));
}
...
inline bool operator>=(const AMAuthId& lhs, const AMAuthId& rhs)
{
    ...
    if(lhs.m_bIsWonid) {
        ...
        if(rhs.m_bIsWonid)
            return false;
        else if(!AMAuthId::same_id_type_class(lhs, rhs))
            return false;
        else
            return ((lhs.m_uiWonid >= rhs.m_uiWonid) && (lhs.m_uiAuthid64 >= rhs.m_uiAuthid64));
    } // if
