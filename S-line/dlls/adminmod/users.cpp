...
auth_struct g_AuthArray[MAX_PLAYERS + 1]; // GLIBC required for that array? [APG]RoboCop[CL]
        ...
        while(iEnd < iLength) {
            ...
            if((iEnd - iBegin) <= 1) {
                ...
                if(*pIncludeFile == '<') {
                    ...
                    while(*pEndTag != '>' && pEndTag < sFile + iEnd)
                        ++pEndTag;
                    ...
                    if(*pEndTag == '"' && pEndTag != pIncludeFile) {
                        ...
                        while(*pPos != '\0') {
                            // skip to the end of the full path, remembering the position of the last directory
                            // seperator
                            if(*pPos == '/') {
                                pLastDir = pPos;
#ifdef WIN32
...
void LoadFile(char* sType, CLinkList<T, isArray>* pList, char* sFileVar, PARSE_FILE fpParse)
{
...
/* -- Support for SQL Dropped -- [APG]RoboCop[CL]
#ifdef USE_MYSQL
...
template<class T, bool isArray> BOOL LoadTable(char* sType, CLinkList<T,isArray>* pList, char* sTableVar, char* sSQL,
PARSE_MYSQL fpParse) {
  int iResult;
...
// if its a user query and we don't want to preload the file exit right now
  ...
  if (iResult) {
    if (mysql_errno(&mysql) == CR_SERVER_LOST || mysql_errno(&mysql) == CR_SERVER_GONE_ERROR) {
      ...
      UTIL_LogPrintf("[ADMIN] ERROR: Select query for %ss returned commands out of sync!  NO FURTHER SQL QUERIES WILL
WORK.\n", sType);
      return FALSE;
    } else {
...
template<class T, bool isArray> BOOL LoadPgTable(char* sType, CLinkList<T,isArray>* pList, char* sTableVar, char* sSQL,
PARSE_PGSQL fpParse) {
  char sQuery[QUERY_BUF_SIZE];
  char* sTable = const_cast<char*>(get_cvar_string_value( sTableVar ));  // TODO: remove necessity for dirty const cast?
  PGresult *pgResult;
...
{
    sprintf(sIP, "%d.%d.%d.%d", (int)((lNbo >> 24) & 0xFF), (int)((lNbo >> 16) & 0xFF), (int)((lNbo >> 8) & 0xFF),
        (int)((lNbo)&0xFF));
}
...
BOOL IsIPValid(const char* sIP)
{
    ...
    if((iDigCount < 1) || (iDigCount > 3) || (iDotCount < 3))
        return FALSE;
    // Otherwise, it's good.
    return TRUE;
}
...
edict_t* get_player_edict(uint32_t _uiID, uidt _uidType)
{
    ...
    switch(_uidType) {
    case uid_index:
        if(_uiID < 1 || _uiID > gpGlobals->maxClients)
            return 0;
    ...
    case uid_sessionID:
        for(i = 1; i <= gpGlobals->maxClients; i++) {
            if(g_AuthArray[i].iSessionID == _uiID)
                return g_AuthArray[i].pPlayerEdict;
        };
        break;

    case uid_wonID:
        for(i = 1; i <= gpGlobals->maxClients; i++) {
            if(g_AuthArray[i].oaiAuthID == _uiID)
                return g_AuthArray[i].pPlayerEdict;
        };
        break;
...
edict_t* get_player_edict(const AMAuthId& _oaiID, uidt _uidType)
{
    ...
    int i;
    for(i = 1; i <= gpGlobals->maxClients; i++) {
        if(g_AuthArray[i].oaiAuthID == _oaiID)
            return g_AuthArray[i].pPlayerEdict;
    };
...
void AddUserAuth(char* sName, char* sIP, edict_t* pEntity)
{
    ...
    DEVEL_LOG(2, ("Prv(idx:%i): Index: %i, SessionID: %i, AuthID: %s, Time: %i, IP: '%s:%i', Name: '%s', Access: %i",
                     iIndex, g_AuthArray[iIndex].iIndex, g_AuthArray[iIndex].iSessionID,
                     (const char*)g_AuthArray[iIndex].oaiAuthID, g_AuthArray[iIndex].iTime, g_AuthArray[iIndex].sIP,
                     g_AuthArray[iIndex].iPort, g_AuthArray[iIndex].sUserName, g_AuthArray[iIndex].iAccess));
    DEVEL_LOG(2, ("Cur(idx:%i): Index: %i, SessionID: %i, AuthID: %s, Time: %i, IP: '%s', Name: '%s'", iIndex, iIndex,
                     iSessionID, (const char*)oaiAuthID, ttiNow, sIP, sName));
    ...
    if(g_AuthArray[iIndex].oaiAuthID == oaiAuthID && g_AuthArray[iIndex].iSessionID == iSessionID &&
        ((ttiNow - g_AuthArray[iIndex].iTime) < 180)
        // && g_AuthArray[iIndex].iPort == iPort
        && strcmp(sName, g_AuthArray[iIndex].sUserName) == 0 && strcmp(sIP, g_AuthArray[iIndex].sIP) == 0) {
    ...
    } else if(g_AuthArray[iIndex].oaiAuthID == oaiAuthID && ((ttiNow - g_AuthArray[iIndex].iTime) < iReconnTime)
        // && g_AuthArray[iIndex].iPort == iPort
        && strcmp(sName, g_AuthArray[iIndex].sUserName) == 0 && strcmp(sIP, g_AuthArray[iIndex].sIP) == 0) {
        DEBUG_LOG(1, ("AddUserAuth: Reconnect for index %i.", iIndex));
        if(iReconnTime > 600) {
            UTIL_LogPrintf(
                "[ADMIN] WARNING: Your reconnect timeout is higher than 600 seconds. This may pose a security risk.\n");
            ...
            if(!oaiAuthID.is_lanid()) {
                ...
                for(int i = 1; i <= gpGlobals->maxClients; i++) {
                    if(g_AuthArray[i].oaiAuthID == oaiAuthID
                        ...
                        && strcmp(sIP, g_AuthArray[i].sIP) == 0 && strcmp(sName, g_AuthArray[i].sUserName) == 0) {
                ...
                for(int i = 1; i <= gpGlobals->maxClients; i++) {
                    if(strcmp(sIP, g_AuthArray[i].sIP) == 0 && g_AuthArray[i].iPort == iPort &&
                ...
                if(poPrevAuth == 0) {
                    for(int i = 0; i < g_AuthBak.SIZE; i++) {
                        if(strcmp(sIP, g_AuthBak.Array[i].sIP) == 0 && g_AuthBak.Array[i].iPort == iPort &&
                            strcmp(sName, g_AuthBak.Array[i].sUserName) == 0) {
                            poPrevAuthBak = &(g_AuthBak.Array[i]);
                            iPrevIndex = i;
                            break;
        ...
        if(poPrevAuth && ((ttiNow - poPrevAuth->iTime) < iReconnectWindow) && !UTIL_PlayerByIndex(iPrevIndex)) {
            ...
            DEBUG_LOG(1, ("AddUserAuth: Reconnect for index %i into new slot %i within %us.", iPrevIndex, iIndex,
                             (ttiNow - poPrevAuth->iTime)));
            // make a back copy of the user data in the slot this user connects into
            DEBUG_LOG(2, ("AddUserAuth: Making backup copy of data for index %i into backup index %i.", iIndex,
                             g_AuthBak.head()));
            ...
            // and his recconection happened within x seconds ( specified in iReconnectWindow )
            DEBUG_LOG(1, ("AddUserAuth: Reconnect for index %i into new slot %i (BAK) within %us", iPrevIndex, iIndex,
                             (ttiNow - poPrevAuthBak->iTime)));
            ...
            if(poPrevAuth) {
                DEBUG_LOG(1, ("AddUserAuth: User %s access rights timed out: %lus, limit:%is.", sName,
                                 (ttiNow - poPrevAuth->iTime), iReconnectWindow));
            } else if(poPrevAuthBak) {
                DEBUG_LOG(1, ("AddUserAuth: User %s access rights (BAK) timed out: %lus, limit:%is.", sName,
                                 (ttiNow - poPrevAuthBak->iTime), iReconnectWindow));
            } else {
...
int GetHighlanderIndex(edict_t* pIgnoreEntity)
{
    ...
    if(pIgnoreEntity != NULL)
        iIgnoreIndex = ENTINDEX(pIgnoreEntity);
    ...
    for(i = 1; i <= gpGlobals->maxClients; i++) {
        ...
        if(i != iIgnoreIndex && g_AuthArray[i].iUserIndex != 0 && IsPlayerValid(pPlayer)) {
            ...
            if(g_AuthArray[i].iAccess > iMaxAccess ||
                (g_AuthArray[i].iAccess == iMaxAccess && g_AuthArray[i].iUserIndex < iMinUserIndex)) {
                iHighlanderIndex = i;
    ...
    if(iHighlanderIndex != m_iHighlanderIndex) {
        ...
        if(iHighlanderIndex != 0) {
            ...
            if(IsPlayerValid(pPlayer)) {
                ClientPrint(
                    &pPlayer->edict()->v, HUD_PRINTTALK, "You are the Admin Highlander! There can be only one!\n");
                CLIENT_PRINTF(
                    pPlayer->edict(), print_console, "You are the Admin Highlander! There can be only one!\n");
            }
        }
...
int GetUserAccess(edict_t* pEntity)
{
    ...
    if(iHighlanderIndex != 0 && iHighlanderIndex != iIndex) {
        return (g_AuthArray[iIndex].iAccess &
            (ACCESS_RESERVE_NICK | ACCESS_RESERVE_SPOT | ACCESS_IMMUNITY | (int)CVAR_GET_FLOAT("default_access")));
        // Otherwise, return the person's access.
    } else {
        // for a listenserver user we add reserved nick and slot access
        return g_AuthArray[iIndex].iAccess |
            (g_AuthArray[iIndex].oaiAuthID.is_loopid() ? (ACCESS_RESERVE_NICK | ACCESS_RESERVE_SPOT) : 0);
    }
}
...
// Returns TRUE if successful, FALSE otherwise.
BOOL GetUserRecord(const char* sName,
    const AMAuthId& oaiAuthID,
    const char* sIP,
    char* sPassword,
    user_struct* ptUserRecord)
{
    ...
    if((int)CVAR_GET_FLOAT("mysql_preload") == 0) {
        ...
        snprintf(sQuery, QUERY_BUF_SIZE, (char*)CVAR_GET_STRING("mysql_users_sql"),
            (char*)CVAR_GET_STRING("mysql_dbtable_users"), escape_chars(sName, '\''), (const char*)oaiAuthID);
        ...
        if(valid_user == FALSE) { // try to search for an IP
            snprintf(sQuery, QUERY_BUF_SIZE, (char*)CVAR_GET_STRING("mysql_users_sql"),
                (char*)CVAR_GET_STRING("mysql_dbtable_users"), sIP, (const char*)oaiAuthID);
        ...
        if(valid_user == FALSE && bTagsTableIsSet) {
            snprintf(sQuery, QUERY_BUF_SIZE, (char*)CVAR_GET_STRING("mysql_tags_sql"),
                (char*)CVAR_GET_STRING("mysql_dbtable_tags"), escape_chars(sName, '\''), (const char*)oaiAuthID);
            if((int)CVAR_GET_FLOAT("admin_debug") != 0)
        ...
        if((int)CVAR_GET_FLOAT("pgsql_preload") == 0) {
            ...
            snprintf(sQuery, QUERY_BUF_SIZE, (char*)CVAR_GET_STRING("pgsql_users_sql"),
                (char*)CVAR_GET_STRING("pgsql_dbtable_users"), escape_chars(sName, '\''), (const char*)oaiAuthID);
            ...
            if(!pgResult) {
                ...
                for(tup_num = 0; tup_num < PQntuples(pgResult); tup_num++) {
                    ...
                    if((int)CVAR_GET_FLOAT("amv_log_passwords") == 1) {
                        DEBUG_LOG(3, ("SQL Query returned password '%s', access '%s'", PQgetvalue(pgResult, tup_num, 0),
                                         PQgetvalue(pgResult, tup_num, 1)));
                    } else {
                        DEBUG_LOG(3,
                            ("SQL Query returned password  '********', access '%s'", PQgetvalue(pgResult, tup_num, 1)));
                    } // if-else
                    if(sPassword == NULL) {
                        // The NULL password matches anything, setup the Users struct and copy it across
                        am_strncpy(User.sUserName, sName, USERNAME_SIZE);
            ...
            if(pg_valid_user == FALSE) { // try to search for an IP
                snprintf(sQuery, QUERY_BUF_SIZE, (char*)CVAR_GET_STRING("pgsql_users_sql"),
                    (char*)CVAR_GET_STRING("pgsql_dbtable_users"), sIP, (const char*)oaiAuthID);
                ...
                if(!pgResult) {
                    ...
                    for(tup_num = 0; tup_num < PQntuples(pgResult); tup_num++) {
                        if((int)CVAR_GET_FLOAT("amv_log_passwords") == 1) {
                            DEBUG_LOG(3, ("SQL Query returned password '%s', access '%s'",
                                             PQgetvalue(pgResult, tup_num, 0), PQgetvalue(pgResult, tup_num, 1)));
                        } else {
                            DEBUG_LOG(3, ("SQL Query returned password  '********', access '%s'",
                                             PQgetvalue(pgResult, tup_num, 1)));
                        }                     // if-else
                        pg_valid_user = TRUE; // there was at least one user...
                        if(sPassword == 0) {
            ...
            if(pg_valid_user == FALSE && bTagsTableIsSet) {
                snprintf(sQuery, QUERY_BUF_SIZE, (char*)CVAR_GET_STRING("pgsql_tags_sql"),
                    (char*)CVAR_GET_STRING("pgsql_dbtable_tags"), escape_chars(sName, '\''), (const char*)oaiAuthID);
                if((int)CVAR_GET_FLOAT("admin_debug") != 0)
                    UTIL_LogPrintf("[ADMIN]Trying tags db with %s\n", sQuery);
    ...
    if(pUser != NULL) {
        if((int)CVAR_GET_FLOAT("admin_debug") != 0) {
            UTIL_LogPrintf("[ADMIN] DEBUG: Name '%s' / AUTHID '%s' matches user entry '%s'\n", sName,
                (const char*)oaiAuthID, ptUser->sUserName);
        }
        if(ptUserRecord != 0) {
        ...
        if(!pResult) {
            ...
            while((pRow = mysql_fetch_row(pResult)) != NULL) {
                ...
                if((ptUserRecord->iAccess & ACCESS_RESERVE_NICK) == ACCESS_RESERVE_NICK) {
                    ...
                    if((int)CVAR_GET_FLOAT("admin_debug") != 0) {
                        UTIL_LogPrintf(
                            "[ADMIN] DEBUG: Name is reserved: Name '%s',  Access '%i'\n", sName, ptUserRecord->iAccess);
                    } // if
                    mysql_free_result(pResult);
                    return TRUE;
                } // if
    ...
    if(sNameToken == NULL) {
        ...
        if(*pcDelim == ':') {
            ...
            *pcDelim = 0;
            ...
            // In that case we have to skip it and search for the next colon.
            if(NULL != pcDelim && AMAuthId::is_authid(sPasswordToken))
                pcDelim = strchr((pcDelim + 1), ':');
            if(NULL == pcDelim)
                sPasswordToken = NULL;
            else {
        ...
        if(sPasswordToken == NULL) {
            ...
            if(sAccessToken == NULL) {
                ...
                */
                ...
                if((int)CVAR_GET_FLOAT("amv_log_passwords") == 1) {
                    DEBUG_LOG(1, ("User loaded: Index: %i, Name '%s', Password '%s', Access '%i'", tUser->iIndex,
                                     tUser->sUserName, tUser->sPassword, tUser->iAccess));
                } else {
                    DEBUG_LOG(1, ("User loaded: Index: %i, Name '%s', Password '********', Access '%i'", tUser->iIndex,
...
BOOL ParseUserSQL(MYSQL_ROW pRow)
{
    ...
    if((int)CVAR_GET_FLOAT("amv_log_passwords") == 1) {
        DEBUG_LOG(1,
            ("User loaded: Name '%s', Password '%s', Access '%i'", tUser->sUserName, tUser->sPassword, tUser->iAccess));
    } else {
...
void SetUserPassword(const char* sName, char* sSetPassword, edict_t* pEntity)
{
    ...
    if(sSetPassword != NULL) {
        ...
        if(pcClientPwBufferCopy != NULL && !FStrEq(pcClientPwBufferCopy, "")) {
            ...
            */
            ...
            /*TBR: I don't think this does anything. Actually, I know that it only works for the info the server has but
    ...
    if(iSetPassword >= 1) {
        ...
        if(iSetPassword < 3) {
            ...
            if((int)CVAR_GET_FLOAT("amv_log_passwords")) {
                DEBUG_LOG(
                    1, ("SetUserPassword: Used previously entered password '%s' for user '%s'.", sPassword, sName));
            } else {
...
void UpdateUserAuth(edict_t* pEntity)
{
    ...
    if(iIndex < 1 || iIndex > gpGlobals->maxClients) {
        UTIL_LogPrintf(
            "[ADMIN] ERROR: UpdateUserAuth: User '%s' has out of bounds entity index %i\n", pEntity->v.netname, iIndex);
        return;
    }
...
BOOL VerifyUserAuth(const char* sName, edict_t* pEntity)
{
    ...
    if(GetUserRecord(sName, AMAuthId(GETPLAYERAUTHID(pEntity)), pcIP, g_AuthArray[iIndex].sPassword, &tUser)) {
        ...
        g_AuthArray[iIndex].iUserIndex = 0;
        DEBUG_DO(1, System_Response(
                        UTIL_VarArgs(
                            "[ADMIN] Wrong or no password for user '%s'.\n[ADMIN] Only default access granted (%i).\n",
                            sName, g_AuthArray[iIndex].iAccess),
                        pEntity););
        if((int)CVAR_GET_FLOAT("admin_debug") != 0) {
            UTIL_LogPrintf(
                "[ADMIN] DEBUG: VerifyUserAuth: User '%s' did not match correct password for any entry.\n", sName);
        }
        fResult = FALSE;
    }
...
int CheckCommand(edict_t* _pEntity,
    const char* _pcCommand,
    unsigned int& _uiAccess,
    BOOL _bCheckUserAcc,
    BOOL _bPrintOut)
{
    ...
    while(pLink != NULL) {
        ...
        if(pPlugin->CheckCommand(_pcCommand, uiAccess)) {
            ...
            if(_bCheckUserAcc && uiAccess != 0 && !(uiAccess & GetUserAccess(_pEntity)))
                continue; // shortcut to next cycle
            ++iResult;
            if(uiAccess < _uiAccess)
                _uiAccess = uiAccess;
            if(_bPrintOut) {
...
plugin_result HandleHelp(edict_t* pEntity, char* sData, int iFormat = 0)
{
    ...
    if(iCount >= iStart) {
        if(iFormat == 1) {
            ...
            System_Response(
                UTIL_VarArgs("----- Entries %i - %i Of %i %s -----\n", iStart, iCount, iMaxCount, sFilterText),
                pEntity);
            ...
            if(iCount < iMaxCount) {
                if(sFilter == NULL) {
                    ...
                    System_Response(
                        UTIL_VarArgs("----- Use 'admin_help %s %i' For More -----\n", sFilter, iCount + 1), pEntity);
                }
            }
        }
...
void LoadVault()
{
    ...
    // Load the vault -- No longer required? [APG]RoboCop[CL]
