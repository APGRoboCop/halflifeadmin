...
AmFSNode::AmFSNode(int _filedes)
    : m_iFiledes(_filedes)
{
    m_stat_node();
    m_poDirectoryObject = NULL;
...
readdiragain:
    if(m_ppoDirEntries == NULL) {
        ...
        if(++m_iCurrDirEntryNum > m_iNumDirEntries)
            return 0;
    }

    if(m_poDirEntry->d_name[0] == '.' && m_poDirEntry->d_name[1] == '\0')
        goto readdiragain;
    if(m_poDirEntry->d_name[0] == '.' && m_poDirEntry->d_name[1] == '.' && m_poDirEntry->d_name[2] == '\0')
        goto readdiragain;
...
int AmDir::m_next_entry(void)
{
    ...
    if(poDirEntry->name[0] == '.' && poDirEntry->name[1] == '\0')
        goto readdiragain;
    if(poDirEntry->name[0] == '.' && poDirEntry->name[1] == '.' && poDirEntry->name[2] == '\0')
        goto readdiragain;
