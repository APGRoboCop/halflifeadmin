    ...
    *pPlugInfo = &Plugin_info;
    ...
    if(!FStrEq(ifvers, Plugin_info.ifvers)) {
        ...
        if(pmajor > mmajor || (pmajor == mmajor && pminor > mminor)) {
            LOG_ERROR(PLID, "Metamod version is too old for plugin %s v.%s; update Metamod", Plugin_info.name,
                Plugin_info.version);
            return (FALSE);
        }
        ...
        // plugin).
            ...
            LOG_ERROR(PLID, "Metamod version is incompatible with plugin %s v.%s; please find a newer version of %s",
                Plugin_info.name, Plugin_info.version, Plugin_info.name);
            return (FALSE);
        }
        ...
        // compatible, so we warn, but we still accept it.
            ...
            LOG_MESSAGE(PLID,
                "WARNING: Metamod version is newer than expected; consider finding a newer version of plugin %s",
                Plugin_info.name);
        else
            LOG_ERROR(PLID,
                "unexpected version comparision; metavers=%s, mmajor=%d, mminor=%d; plugvers=%s, pmajor=%d, pminor=%d",
                ifvers, mmajor, mminor, META_INTERFACE_VERSION, pmajor, pminor);
    }
    return (TRUE);
}
...
C_DLLEXPORT int GetEngineFunctions_Post(enginefuncs_t* pengfuncsFromEngine, int* interfaceVersion)
{
    if((int)CVAR_GET_FLOAT("developer") != 0)
        UTIL_LogPrintf(
            "[%s] DEBUG: called: GetEngineFunctions_Post; version=%d\n", Plugin_info.logtag, *interfaceVersion);
    if(!pengfuncsFromEngine) {
        UTIL_LogPrintf(
            "[%s] ERROR: GetEngineFunctions_Post called with null pengfuncsFromEngine\n", Plugin_info.logtag);
        return (FALSE);
...
gFunctionTable = {
    ...
    GetGameDescription, // pfnGetGameDescription    Returns string describing current .dll game.
    NULL,               // PlayerCustomization,		//pfnPlayerCustomization   Notifies .dll of new customization for
                        // player.
    ...
    NULL, // SpectatorThink,		//pfnSpectatorThink        Called when spectator sends a command packet
          // (usercmd_t)

    NULL, // Sys_Error,			//pfnSys_Error				Called when engine has
          // encountered an error

    NULL, // PM_Move,			//pfnPM_Move
    NULL, // PM_Init,			//pfnPM_Init				Server version of player movement
          // initialization
    NULL, // PM_FindTextureType,		//pfnPM_FindTextureType

    NULL, // SetupVisibility,		//pfnSetupVisibility        Set up PVS and PAS for networking for this
          // client
    NULL, // UpdateClientData,		//pfnUpdateClientData       Set up data sent only to specific client
    NULL, // AddToFullPack,		//pfnAddToFullPack
    NULL, // CreateBaseline,		//pfnCreateBaseline			Tweak entity baseline for network
          // encoding, allows setup of player baselines, too.
...
NEW_DLL_FUNCTIONS gNewFunctionTable = {
    OnFreeEntPrivateData, //! pfnOnFreeEntPrivateData()	Called right before the object's memory is freed.  Calls its
                          //! destructor.
