...
static void *labels[] = {
        ...
        &&op_swap_alt,  &&op_pushaddr,  &&op_sleep };
  AMX_HEADER *hdr;
  AMX_FUNCSTUB *func;
  uchar *code, *data;
  ...
  op_retn:
    ...
    POP(offs);
    cip=(cell *)(code+(int)offs);
    stk+= *(cell *)(data+(int)stk) + 4; /* remove parameters from the stack */
    amx->stk=stk;
    if (offs==0) {
      /* entry function returns -> end of program */
      if (retval!=NULL)
  ...
  op_sleep:
    ...
    amx->cip=(uchar*)cip-code;
    #if !defined NODBGCALLS
      if (debug) {
        amx->dbgcode=DBG_SLEEP;
        //amx->dbgaddr=(ucell)(cip-code);
        amx->debug(amx);
      } /* if */
    #endif
...
int AMXAPI amx_Exec(AMX *amx, cell *retval, int index, int numparams, ...)
{
  ...
  if (debug) {
     ...
     */
    amx->dbgcode=DBG_CALL;
    amx->dbgaddr=(ucell)((uchar *)cip-code);
    amx->debug(amx);
  } /* if */
