 ...
 *
 *  $Id: amxstring.c,v 1.2 2001/03/22 01:00:17 darope Exp $
 *
 ****************************************************************/
...
*/
int get_space( char** _string, size_t _iMinLen, size_t* _piStrLen ) 
{
  char* pcNewstr = NULL;
  int i;
  int iLocalFree = 0;
...
*/
size_t str_space( const char* _pcPointer ) {
...
*/
int bind_string( AMX* _amx, char** _string, cell _cstrAddress, size_t* _piStrLen, size_t _iMinLen ) {
...
*/
int get_string( AMX* _amx, char** _string, cell _cstrAddress, size_t* _piStrLen, size_t _iMinLen ) {
...
*/
int set_string( AMX* _amx, cell _cstrAddress, char* _string, size_t _iMaxLen ) {
...
*/
int string_len( AMX* _amx, cell _cstrAddress, size_t* _iStrLen ) {
  ...
  /* set the Small string */
  err = amx_StrLen( ptCellString, _iStrLen );
  if ( err != AMX_ERR_NONE ) {
    return err;
  }  // if
...
*/
int clear_string( AMX* _amx, cell _cstrAddress, size_t _iStrLen ) {
...
const char* _strgtok( char* _pcString, char* _pcDelimiter, char* _pcGrouping ) {
...
static cell AMX_NATIVE_CALL amx_strcpy( AMX* amx, cell* params )
{
  ...
  err = get_string( amx, &pcFrom, params[2], &iFromLength, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  if ( params[3] ) {
    iMaxLength = min( (params[3]-1), iFromLength );
  }  // if
...
static cell AMX_NATIVE_CALL amx_strcmp( AMX* amx, cell* params )
{
  ...
  /* first get the source!! */
  err = get_string( amx, &pcString2, params[2], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  /* allocate memory long enough for the source!! */
  err = get_string( amx, &pcString1, params[1], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strchr( AMX* amx, cell* params )
{
  ...
  /* first get the source!! */
  err = get_string( amx, &pcString, params[1], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strstr( AMX* amx, cell* params )
{
  ...
  /* get the needle */
  err = get_string( amx, &pcSubString, params[2], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strstrx( AMX* amx, cell* params )
{
  ...
  /* get the haystack */
  err = get_string( amx, &pcString, params[1], &iStringLen, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  /* get the needle */
  err = get_string( amx, &pcSubString, params[2], &iSubStringLen, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strspn( AMX* amx, cell* params )
{
  ...
  /* get the skipset */
  err = get_string( amx, &pcSkipSet, params[2], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strcspn( AMX* amx, cell* params )
{
  ...
  /* get the stopset */
  err = get_string( amx, &pcStopSet, params[2], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strtok( AMX* amx, cell* params )
{
  ...
  /* clear the token */
  err = clear_string( amx, params[3], 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  /* get the delimiters */
  err = get_string( amx, &pcDelimiters, params[2], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strgtok( AMX* amx, cell* params )
{
  ...
  /* clear the token */
  err = clear_string( amx, params[4], 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  /* get the grouping */
  err = get_string( amx, &pcGrouping, params[3], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strgsplit( AMX* amx, cell* params )
{
  ...
  /* get the string */
  err = get_string( amx, &pcString, params[1], &tStringSize, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  /* get the grouping set */
  err = get_string( amx, &pcGroupingSet, params[3], NULL, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strcount( AMX* amx, cell* params )
{
  ...
  /* first get the source!! */
  err = get_string( amx, &pcString, params[1], &iCount, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strtrim( AMX* amx, cell* params )
{
  ...
  err = get_string( amx, &pcString, params[1], &tiStringLength, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  /* get the trim set */
  err = get_string( amx, &pcTrim, params[2], &tiNumTrims, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  /* set the string in the VM */
  err = set_string( amx, params[1], pcString, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_strsubst( AMX* amx, cell* params )
{
  ...
  /* first get the string */
  err = get_string( amx, &pcString, params[1], &tiStringLen, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  err = get_string( amx, &pcSearch, params[2], &iSearchLen, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  ...
  err = get_string( amx, &pcSubst, params[3], &iSubstLen, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
...
static cell AMX_NATIVE_CALL amx_snprintf( AMX* amx, cell* params )
{
  ...
  err = get_string( amx, &pcFormat, params[3], &iRetVal, 0 );
  if ( err != AMX_ERR_NONE ) {
    RAISE_ERROR(err);
    return Return((cell)AMXFAIL);
  }  // if
  
  pcEndOfString = pcFormat + iRetVal;
  ...
  while ( (tCharsLeft > 0)  && (pcStart < pcEndOfString) ) {
	...
	tSkip = strspn( pcEnd, "#0+-.123456789" );
	pcEnd += tSkip;
	...
	case 's':
	  ...
	  err = get_string( amx, &pcStringArg, vlist[iNextParam], NULL, 0 );
	  if ( err != AMX_ERR_NONE ) {
	    RAISE_ERROR(err);
	    return Return((cell)AMXFAIL);
	  }  // if
	  ...
	  *pcEnd = 0;
	  ...
	  if ( iRetVal > -1 && iRetVal < tCharsLeft ) {
	    tCopy = iRetVal;
	  } else {
	    tCopy = tCharsLeft;
	  } // if-else
	...
	case 'o':
	  ...
	  *pcEnd = 0;
	  ...
	  iRetVal = snprintf( pcOutput, (tCharsLeft+1), pcStart, (int)*ptCell );
	  if ( iRetVal > -1 && iRetVal < tCharsLeft ) {
	    tCopy = iRetVal;
	  } else {
	    tCopy = tCharsLeft;
