 ...
 * ===========================================================================
 *
 * $Id: admin_mod.cpp,v 1.12 2003/11/08 10:21:22 darope Exp $
 *
 *
...
int AM_ClientCommand( edict_t *pEntity ) {
  ...
  if ( CMD_ARGS() ) strncpy( commandline, CMD_ARGS(), 255 );
    ...
    /* builtin commands are now registered in the tree along with plugin commands,
     * so no seperate hard-coded checks here anymore */
  
    /* also not supporting script_file at the moment */
...
BOOL AM_ClientConnect( edict_t *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[ 128 ], bool bForce ) {    
 ...
 if ( pszAddress ) {
  ...
  if ( (!(int)CVAR_GET_FLOAT("sv_lan")) &&  AMAuthId::is_pending(pcAuthId) ) {
	  //UTIL_LogPrintf( "|---> Player Steam Id (ClientConnect) is pending.\n" );
	  ...
	  if ( !g_ovcPendingPlayers.empty() ) {
          ...
          if ( !bQueued ) {
        	  ...
        	  //UTIL_LogPrintf( "|---> Player Steam Id (ClientConnect) Enqueued player (%p).\n", pEntity );
          }
	  } else {
          // Enqueue player
          g_ovcPendingPlayers.push_back( poPlayerData.release() );
  ...
  if ( (int)CVAR_GET_FLOAT("amv_private_server") != 0 ) {
  ...
  if (IsNameReserved(sName, oaiAuthID, sIP, &tUser)) {
  ...
  // check if the model is reserved
...
void AM_ClientStart(edict_t *pEntity) {
  ...
  if ( g_fUseMySQL == FALSE && sHost != NULL && sUser != NULL) {
    g_fUseMySQL = TRUE;
    mysql_init( &mysql );
    mysql_real_connect( &mysql, sHost, sUser, sPassword, sDatabase, 0, NULL, 0 );
...
/* Via a function table, this ends up calling GameDispatchSpawn which does
   * nothing at the moment, so this need not be done
...
*/
 ...
 char acKickMsg[128]; 
  ...
  if(!FStrEq(sName, STRING(pEntity->v.netname)) || bForce) {
	  //TBR SetUserPassword(sName, NULL, pEntity);
	  ...
	  if (IsNameReserved(sName, oaiAuthID, 0, &tUser)) {
		  if ( ( (tUser.iAccess >= 0) && ( ((GetUserAccess(pEntity) & ACCESS_RESERVE_NICK) != ACCESS_RESERVE_NICK) 
			  ...
			  if ( sNickRejectMsg == NULL ) {
				  ...
				  am_strncpy( acKickMsg, "[ADMIN] Your name or WON ID is reserved on this server.", 128 );
			  } else {
				  CLIENT_PRINTF(pEntity, print_console, UTIL_VarArgs("%s\n",sNickRejectMsg));
				  am_strncpy( acKickMsg, sNickRejectMsg, 128 );
			  }
			  DEBUG_LOG( 1, ("Access to nickname denied for user '%s'", sName) );
			  iKick = 1;
		  } else {
	  ...
	  if (IsModelReserved(sModel)) {
		  if (GetModelAccess(sModel, pEntity)==0) {
			  ...
			  if ( sModelRejectMsg == NULL ) {
				  ...
				  am_strncpy( acKickMsg, "[ADMIN] That model is reserved on this server.", 128 );
			  } else {
				  CLIENT_PRINTF(pEntity, print_console, UTIL_VarArgs("%s\n",sModelRejectMsg));
				  am_strncpy( acKickMsg, sModelRejectMsg, 128 );
			  }
			  iKick = 1;
			  DEBUG_LOG( 1, ("Access to model '%s' denied for user '%s'", sModel, sName) );
		  } else {
  ...
  if (iKick == 1) {
    if ( !GETPLAYERUSERID(pEntity) ) { // whoops, can't find seesionid
    	...
    	UTIL_LogPrintf( "[ADMIN] Kicked %s due to bad nickname/model info for user %s\n", STRING(pEntity->v.netname), g_engfuncs.pfnInfoKeyValue( infobuffer, "name"));
    	snprintf( szCommand, 256, "kick #%i %s\n", GETPLAYERUSERID(pEntity), acKickMsg );
...
int AM_GetGameDescription( const char* _pcDescription ) {
  ...
  if ( (iResSlots != s_iResSlots) || (iHideSlots != s_iHideSlots) 
	   || (iVisMaxPlayers != s_iVisMaxPlayers) || (iPlayerCnt != s_iPlayerCnt)
	   || (iFreeSlots != s_iFreeSlots) ) {
...
int AM_StartFrame( void ) {
	...
	if ( s_uiFCount % 500 == 0 ) {  // execute every X frames
		...
		if ( !g_ovcPendingPlayers.empty() ) {
		    //UTIL_LogPrintf( "|---> (Start Frame) Checking pending player queue. Queue length: %d\n", g_ovcPendingPlayers.size() );
			...
			if ( AMAuthId::is_pending(pcAuthId) ) {
				...
				//UTIL_LogPrintf( "|---> (Start Frame %lu) Auth id validated (%s), calling ClientUserInfoChanged()\n", s_uiFCount, pcAuthId );

				// Run SetUserPassword() since we might have an invalid entry from a first connect
//printf( "|----> Call SetUserPassword( %s, NULL, %p )\n", pePPlayer->acName, pePPlayer->pEntity );
...
//printf( "|----> Call SERVER_COMMAND( kick #%i %s )\n",  GETPLAYERUSERID(pePPlayer->pEntity), acRejectMsg );
  ...
  if ( iPlayerIndex != 0) {
    ...
    if ( sReserveKickMsg == NULL ) {
      CLIENT_PRINTF( pPlayer->edict(), print_console, "[ADMIN] The Server Admin has enabled reserved slots. You have been kicked due to another player taking a reserved slot\n");
    	snprintf(szCommand,BUF_SIZE,"kick #%i %s\n", GETPLAYERUSERID(pPlayer->edict()), "[ADMIN] The Server Admin has enabled reserved slots. You have been kicked due to another player taking a reserved slot." );
    } else {
      CLIENT_PRINTF( pPlayer->edict(), print_console, sReserveKickMsg);
    	snprintf(szCommand,BUF_SIZE,"kick #%i %s\n", GETPLAYERUSERID(pPlayer->edict()), sReserveKickMsg );
    }
