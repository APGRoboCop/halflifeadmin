 ...
 * ===========================================================================
 *
 * $Id: users.cpp,v 1.13 2003/11/08 10:21:24 darope Exp $
 *
 *
...
template<class T, bool isArray> void LoadFile(const char* sType, CLinkList<T,isArray>* pList, const char* sFileVar, PARSE_FILE fpParse) {
...
void AddUserAuth(char* sName, char* sIP, edict_t* pEntity) {
  ...
  if ( iReconnectWindow != 0 ) {
      ...
      DEBUG_LOG( 2, ("AddUserAuth: Making backup copy of data for index %i.", iIndex) );
      memcpy( &(g_AuthBak.Array[g_AuthBak.head()]), &g_AuthArray[iIndex], sizeof(auth_struct) );
  }
...
plugin_result HandleCommand(edict_t* pEntity, char* sCmd, char* sData) {
  ...
  plugin_result r= g_Dispatcher.Dispatch(pEntity,sCmd,sData);
  return r;
}
...
BOOL ParsePlugin( char* sLine ) {
  ...
  if ( oNode.is_directory() ) {
	...
	while ( NULL != (pcNode = poDir->get_next_entry(oNode)) ) {
		// Check if this is a file with ending '.amx'
		if (oNode.is_file() && strcasecmp(pcNode+strlen(pcNode)-4, ".amx") == 0 ) {
  ...
  if(pPlugin->LoadPlugin( sPlugin, m_pPluginList->Count()+1 )) {
