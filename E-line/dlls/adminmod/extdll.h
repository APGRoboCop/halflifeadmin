...
*
*   $Id: extdll.h,v 1.14 2003/11/08 10:21:23 darope Exp $
*
****/
#ifndef EXTDLL_H
#define EXTDLL_H
...
static inline void handle_unprintables( char* _pcString ) {  
	if ( g_PreventClientExploit != 0 ) {
		while ( *_pcString != '\0' && *_pcString != '\r') {
   			if ( !isprint(*_pcString) ) *_pcString = '.';
			++_pcString;	
		}  // while

		//Remove rogue carriage returns from when a command is in
		//a cfg file and the cfg is saved in DOS format
		*_pcString = '\0';
	}  // if  
}  // handle_unprintables() 
...
int make_friendly(char *, BOOL check = false);
