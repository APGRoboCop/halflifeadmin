 ...
 * ===========================================================================
 *
 * $Id: admin_commands.cpp,v 1.17 2003/11/08 10:21:21 darope Exp $
 *
 *
...
//Wrapper arround SystemError to include the plugin name where possible
void Runtime_Error(AMX* amx, const char* strMsg) {
	char buf[1000];
	strncpy(buf,strMsg,1000);

	CPlugin* pPlugin = GetPlugin(amx);
  if (pPlugin == NULL) {
    System_Error(UTIL_VarArgs("[ADMIN] ERROR: %s\n",buf),pAdminEnt);
  }
 
  if (pPlugin->Name() == NULL) {
    System_Error(UTIL_VarArgs("[ADMIN] Error in %s: %s\n", pPlugin->File(), buf),pAdminEnt);
  } else {
    System_Error(UTIL_VarArgs("[ADMIN] Error in %s: %s\n", pPlugin->Name(), buf),pAdminEnt);
  }
}
...
#define CHECK_AMX_PARAMS(Required) \
	if (iNumParams < Required) { \
		Runtime_Error(amx,UTIL_VarArgs("[ADMIN] ERROR: Missing required parameters.  Required %i, has %i.\n", Required, iNumParams)); \
...
#define GET_AMX_STRING(Param,MaxLength,String,Truncate) \
	...
	if (iLength >= MaxLength) { \
          if ( Truncate == 0 ) { \
	    Runtime_Error(amx,UTIL_VarArgs("[ADMIN] ERROR: AMX string too large.  Max size %i, is %i.\n", MaxLength, iLength)); \
	    amx_RaiseError(amx,AMX_ERR_NATIVE); \
	    return 0; \
          } else { \
	    Runtime_Error(amx,UTIL_VarArgs("[ADMIN] WARNING: AMX string too large. Length is %i, truncating to %i.\n", iLength, MaxLength)); \
...
#define SET_AMX_STRING(Param,MaxLength,String) \
	...
	if (String == NULL) { \
		...
		Runtime_Error(amx,UTIL_VarArgs("[ADMIN] ERROR: AMX string too small.  Required size %i, is %i.\n", strlen(String), MaxLength)); \
		amx_RaiseError(amx,AMX_ERR_NATIVE); \
		return 0; \
	} else { \
...
static cell plugin_checkcommand(AMX* amx, cell* params) {
  ...
  iResult = g_Dispatcher.CheckCommand( pAdminEnt, acCommand, iAccess, TRUE, FALSE );
...
static cell vote_multiple(AMX *amx, cell *params) {
  ...
  CTimer *pEntity = (CTimer *)GET_PRIVATE(pTimerEnt);  
  if (pEntity==NULL) {
    Runtime_Error(amx,"[ADMIN] ERROR: Attempt to start a vote when no map is loaded.\n");
    amx_RaiseError( amx,AMX_ERR_NATIVE );
    return 0;
  }

  if (!pEntity->StartVote(amx,sText,iChoiceCount,iBits,sFunction,sParam,pAdminEnt)) {
    Runtime_Error(amx,"[ADMIN] ERROR: CTimer::StartVote failed.\n");
    //amx_RaiseError(amx,AMX_ERR_NATIVE);
    return 0;
  } 
  return 1;
...
#define FSUNIT_LINES 1
  ...
  if ( get_file_path(acFilePath, acFilename, PATH_MAX, "file_access_read") > 0 ) {
    g_Handles.FlushRead(acFilePath);
	g_Handles.FlushWrite(acFilePath);
    ...
    if( (fFile = fopen(acFilePath,"r")) == NULL ) {
      return 0;
		...
		while (fgets(acLine, BUF_SIZE, fFile)) {
			if (strchr(acLine,'\n')) {
				iNumLines++;
			}
			iNumBytes+=strlen(acLine);
		}
    fclose( fFile );
...
static cell readfile(AMX *amx, cell *params) {
  ...
  if (g_Handles.ReadLine(acFilename,line_no,acLine)) {
    if ( strlen(acLine) >= 0 ) {
      amx_GetAddr( amx, params[2], &cptr );
...
static cell writefile(AMX *amx, cell *params) {
  ...
  amx_GetAddr( amx, params[2], &cStr );
  amx_StrLen( cStr, &iLen );
  if ( iLen >= LARGE_BUF_SIZE ) {
    DEBUG_LOG( 2, ("ERROR:writefile: max. line size is %d, provided size is %d..", LARGE_BUF_SIZE, iLen) );
    amx_RaiseError( amx, AMX_ERR_NATIVE );
    return 0;
  }
  amx_GetString( acData, cStr );

  if (g_Handles.WriteLine(acFilename,params[3],acData)) {
	  return 1;
  }
  return 0;
...
static cell spawn(AMX *amx, cell *params) {
  ...
  /* Nothing happens in GameDispatchSpawn at the moment
...
// register_event - makes this plugin register to recieve events of the
//   class "event"
static cell plugin_registerevt(AMX* amx, cell* params) {
  int iLength;
  int iNumParams = params[0] / sizeof(cell);
  char event[BUF_SIZE];
  char function[BUF_SIZE];
  cell* cStr;
  CPlugin* pPlugin = GetPlugin(amx);

  if (pPlugin == NULL)
  {
	System_Error("[ADMIN] ERROR: plugin_registerevt: Could not find matching amx.\n",pAdminEnt);
    	amx_RaiseError(amx,AMX_ERR_NATIVE);
	return 0;
  }
  CHECK_AMX_PARAMS(2);
  GET_AMX_STRING(1,BUF_SIZE, event,0);
  GET_AMX_STRING(2,BUF_SIZE, function,0);

  if (pPlugin->AddEvent( event, function )) {
      return 1;
  }
  else {
	  return 0;
...
/*
static cell cevent_getkey_string(AMX* amx, cell* params) {
  int iLength;
  int iNumParams = params[0] / sizeof(cell);
  cell* cStr;
  char key[BUF_SIZE];
  ...
  CEvent *ev = (CEvent *)( params[1] );
  GET_AMX_STRING(2,BUF_SIZE,key,0);

  if( !ev || !key )
  {
    System_Response( "[ADMIN] You must pass in an event handler and key\n",pAdminEnt);
    return 0;
  }

  DEBUG_LOG(4, ("cevent_getkey_string() ev: %p", ev) );

  iLength = params[4];
  SET_AMX_STRING(3, iLength, ev->GetKey_String(key) );

  return 1;
}

static cell cevent_getkey_int(AMX* amx, cell* params) {
  int iLength;
  int iNumParams = params[0] / sizeof(cell);
  cell* cStr;
  char key[BUF_SIZE];
  ...
  DEBUG_LOG(4, ("cevent_getkey_int() ev: %p", ev) );

  return ev->GetKey_Int(key);
}

static cell cevent_getkey_float(AMX* amx, cell* params) {
  int iLength;
  int iNumParams = params[0] / sizeof(cell);
  cell* cStr;
  char key[BUF_SIZE];
  ...
  DEBUG_LOG(4, ("cevent_getkey_float() ev: %p", ev) );

  return (static_cast<cell>( ev->GetKey_Float(key) ));
}
*/
...
AMX_NATIVE_INFO admin_Natives[] = {
  ...
  { "motd", motd},
  { "plugin_registerevt", plugin_registerevt},
  // Terminator
  { 0, 0 }
};
